...
- **Don't say it twice.** When you add a marker, delete the hand-written "only available in the Pro and Enterprise+ editions" sentence or note it replaces. The callout already links to `/pricing/`. Keep only prose that says something the callout doesn't (for example, a per-edition limit like "Pro allows up to 25 custom roles").
- The editions are Free, Essentials, Pro, and Enterprise+ (September 2026; they were Individual, Team, Enterprise, and Business Critical). Names are lowercase-noun in prose ("the Enterprise+ edition"). Never "Starter", and never bare "Enterprise" for an edition — it's ambiguous between the old Enterprise edition, now Pro, and Enterprise+. "Pro" was also a pre-2023 grandfathered SKU; say "the legacy Pro SKU" when you mean that one.
...
- **Optional `editions:`** is a YAML array marking Pulumi Cloud edition availability, rendered as badge(s) beside the date. Values are edition **ids** from the closed set in `data/pulumi_pricing.yaml` (`free`, `essentials`, `pro`, `enterprise-plus`), enforced by `checkChangelogEditions` in `scripts/lint/lint-markdown.js`; the templates look the id up and render the display name, so write `enterprise-plus`, not `Enterprise+`. List **every** edition the feature is available in — since a lower edition implies the ones above it, that means the lowest applicable edition and all editions above it (e.g. a Pro feature lists both `pro` and `enterprise-plus`). The legacy `tiers:` array and singular `tier:` scalar are hard build failures: "tier" isn't a word the product uses, and the old list predates editions entirely.
