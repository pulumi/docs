...
- **Link to stable Dev Center pages, not individual tutorials.** Prefer facet landing pages (`/dev/browse/cloud/aws/`), filter URLs (`/dev/browse/?cloud=aws&tag=containers`), hubs, and template groups (`/dev/templates/<group>/[<cloud>/]`). Link to a single `/dev/tutorials/<slug>/` only when it's the page's hands-on companion; tutorials can be retired in marketing-web without this repo knowing. Full rules: `STYLE-GUIDE.md` §Linking to the Dev Center. Never `/tutorials/` or `/templates/`; those only redirect.
...
- **Don't say it twice.** When you add a marker, delete the hand-written sentence or note that lists the required editions. The callout already links to `/pricing/`. Keep only prose that says something the callout doesn't, such as a per-edition limit.
- The V6 editions are Free, Essentials, Pro, and Enterprise. Use those names for V6 product guidance. Preserve V5 edition names when content specifically describes existing V5 customers. "Pro" was also a pre-2023 legacy plan; say "the legacy Pro plan" when you mean that one.

### Pricing data

`data/pulumi_pricing.yaml` is the single source of truth for **what Pulumi sells**: the ordered, closed set of editions (Free, Essentials, Pro, Enterprise) with the display copy for their `/pricing/` cards, and a per-feature availability matrix. Its header comment is authoritative for the vocabulary ("edition", never "plan"/"tier"/"subscription") and for adding or renaming an edition or a feature. Everything downstream reads it: the `/pricing/` comparison table, the docs availability markers above, and the changelog `editions:` badges.
...
- **`requires:`** names the edition a marker should state when it differs from the first edition with a truthy cell — needed wherever the free column is a *limited variant* rather than the feature itself ("Manual" policy enforcement at Free). Setting it also makes the cells below it exempt from the monotonicity rule, since they are variants rather than availability.
...
- **Optional `editions:`** is a YAML array marking Pulumi Cloud edition availability, rendered as badge(s) beside the date. Values are edition **ids** from the closed set in `data/pulumi_pricing.yaml` (`free`, `essentials`, `pro`, `enterprise`), enforced by `checkChangelogEditions` in `scripts/lint/lint-markdown.js`; the templates look the id up and render the display name, so write `enterprise`, not `Enterprise`. List **every** edition the feature is available in — since a lower edition implies the ones above it, that means the lowest applicable edition and all editions above it (e.g. a Pro feature lists both `pro` and `enterprise`). The legacy `tiers:` array and singular `tier:` scalar are hard build failures: "tier" isn't a word the product uses, and the old list predates editions entirely.
...
**The v3 surface (on repo-wide):** `REVIEW_V3_COMMENTS` is set for pulumi/docs, so every fresh review renders v3 cards; a PR reviewed before the flip keeps its legacy monolith until someone comments `@claude #new-review`, since `#update-review` refreshes v2 in place by design. On v3 the monolithic pinned review becomes an author card (`<!-- CLAUDE_REVIEW_AUTHOR -->`, blocking findings with stable `F<n>` ids + a machine-read `REVIEW_STATE` disposition block) plus a reviewer brief (`<!-- CLAUDE_REVIEW_BRIEF -->`), with the verification trail on a linked evidence page and S3 as the system of record. Findings are answered by pushing a fix or replying `@claude <reasoning> #update-review` (agents may also use the deterministic `/resolve F<n> <disposition>[: reason]` comment lane — zero model cost, not surfaced to human contributors); the **Sentinel** check-run is the single merge gate (findings answered · matrix-named team approved per `.github/review-routing.yml` — every PR routes to a team, including bot-authored ones; `mechanical` skips the model review, never the approver · a staging deploy for changes to the deploy itself, per that file's `staging_evidence.paths`), with `review:waived` as the logged break-glass. Deterministic machinery lives in `scripts/review-v3/` (see its README) and is covered by `make test-review-pipeline`.

**Review mode vs fix mode.** The pre-step artifacts are shaped for *a human reading a comment*, and a consumer that applies edits instead of posting them needs different settings. `vale-findings-filter.py` is the live example: by default it caps findings at 10 per file / 50 total and de-duplicates advisory findings per `(category, message)`, all so the pinned review stays readable. Those are a comment budget, not an analysis budget. A lane that fixes rather than flags must pass `--fix-mode` (today: `content-review-article.yml`, both the content-review and glow-up lanes) or it silently clears the first ten findings and leaves the rest to resurface as churn on the next review. The dedup fails a fixer the same way: a rule whose message names a *shape* rather than a term (`Pulumi.NarrativeWe` — one message for every "we will" in the file) collapses to one line and hides the other occurrences. If you add a consumer that applies findings, check every artifact it reads for the same distinction before assuming the defaults fit.
