...
- **Whole page**: add `pulumi_cloud: <feature-id>` to the front matter (for example `pulumi_cloud: rbac`). `layouts/docs/{single,list}.{html,md}` renders the callout above the content. We only mark what a reader has to buy, so an unknown id, an *edition* id, `true`, `false`, and a feature that's available on the Individual edition are all hard lint failures (`checkPulumiCloudValue` in `scripts/lint/lint-markdown.js`). An ungated page carries no key.
...
- **If the feature isn't in the data file yet, add it there first.** Give it `hidden: true` when it isn't a marketed line item on `/pricing/` (gated deployments, ESC change requests, customer-managed runners): hidden features stay out of the comparison table but remain resolvable by id.
