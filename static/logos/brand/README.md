...
- **Removing a file?** Files here are served publicly (at `/logos/brand/…`) and
  may be hotlinked from outside the repo. Before deleting, check the codebase for
  references and add a redirect under `scripts/redirects/` so old URLs keep
  working. See the repo root `AGENTS.md` for redirect conventions.
