...
A run does not stop at a rendering. The whole flow is collect → analyze → judge → publish the board → **plan** → one question: execute it? Planning is read-only (step 5 says why), so it is a step you perform, never a step you propose. If decisions exist and `.pr-review-plan.json` doesn't, the run isn't finished — keep going.

**Every question this skill asks goes through `AskUserQuestion`**: the execute confirmation, the per-row walk in `--terminal`, and anything else that comes up. Never end a turn on a question typed into prose — that is a turn the person has to restart by hand, and it costs them the round trip the tool exists to avoid. If nothing genuinely needs deciding, act and report.
...
**Planning is read-only, so never ask permission to plan.** It writes one local file and prints the preview; it makes no GitHub call and changes nothing. Run it as soon as the decisions exist, for every decision that exists — do not ask which subset to plan, and do not offer "plan all" versus "plan some" as a choice. "Want me to plan it? Planning is read-only" is the exact failure this paragraph exists to prevent: reciting the reason the question is unnecessary and then asking it anyway still ends the turn and still costs a round trip. There is exactly one question in this step, it is asked with `AskUserQuestion`, and it comes *after* the preview is on screen: **execute this plan?** Its options are about doing the writes — run it, run it minus specific PRs, or stop — and the preview the person just read is what they are answering about. An explicit `--act` in the user's own invocation is already that yes for exactly that plan; a re-plan needs a fresh one. Then execute, and report the per-step results.

- `--stamp N,N` — per PR, immediately before merging, re-fetch the PR: head unchanged since the plan, `mergeable_state` in {clean, blocked}, checks green, no changes-requested review. Then post one `/resolve F<n> <disposition>: <why>` per judged finding (`fixed`, `refuted`, `accepted`, `not-applicable`; `deferred` is the author's and goes back instead, and a triage-prose id is not a card finding), approve (one line per `pr-review:references:message-templates`, no footer) and squash-merge. The resolves land first, so the record of why each finding closed never loses a race with the merge. A bot PR merges — dependabot, pulumi-bot and WorkPrentice alike, since they exist to be merged; a human-authored PR is approved only, because it is the author's to merge. `N:merge` and `N:no-merge` override that default for one PR (`--stamp 21550,21577:no-merge`), so a mixed batch stays one command; the blanket `--merge-humans` and `--no-merge` still apply where no per-PR mode is given. A failed preflight skips that PR and the batch continues. `--force` stamps a judge row (approve as-is); a blocked row is never stampable.
