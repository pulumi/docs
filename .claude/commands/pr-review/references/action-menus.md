...
| `blocked` | the unblock: `--unblock N` (dirty or behind), `--refresh N` (stale review), `--rerun N` (errored review), `--rerun-checks N` (red checks that look like flake), `--close N --superseded-by M` (duplicate) | for open 🚨 findings, `--request-changes N` (send back to author) or `--close N` on an `author:generated` row; `--route N:@owner` when the row is another lane's; `open PR`. A blocked row that ends up with no unblock at all shows "Blocked: … no action available" and is counted on its own tally tile |
