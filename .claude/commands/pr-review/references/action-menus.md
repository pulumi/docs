...
Approving a judged row posts the approval and the merge, nothing else: the judgments are the approver's notes on the board, and they never answer a finding on the author card. A `deferred` judgment is what the send-back (or close) button is for.
