...
| 2 | `docs-review:references:blog` | `content/blog/**`, `content/customers/**` |
...
**Rule 5 is one review lens and two labels.** `theme/src/scss/` and `.github/workflows/` both review under `docs-review:references:infra` — Hugo correctness, the dark-mode pass, fingerprinted images, and build-pipeline criteria are the same lens either way. The label split exists for the v3 approval matrix (`.github/review-routing.yml`), not for review criteria: `domain:infra` is the build and deploy pipeline, approved by the tools team; `domain:frontend` is how the site looks, approved by marketing. Neither label decides whether a staging run is required — that is a third, independent question answered by `staging_evidence.paths`, which covers the deploy chain and nothing else. Changing the lens for `domain:frontend` files would mean reviewing a Hugo partial for voice and tone while nobody checks the template, so the lens stays infra.
