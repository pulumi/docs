...
Workflow pre-step: `extract-claims.py` (a deterministic regex floor — numbers, version pins, temporal words, source attributions, URLs, named-entity/spec claims, positioning/comparison trigger words) ∪ two redundant Sonnet passes `extract-claims-llm.py` (one atomic/per-sentence-framed, one holistic/paragraph-framed, both prompted with `docs-review:references:claim-extraction`) → unioned and deduped by `merge-claims.py` into `.candidate-claims.json` at the repo root: `{"claims": [{"file", "line_range", "text", "type", "source_hint"?, "confidence", "found_by": [...], "line_range_unverified"?}], "stances": [...same shape, type positioning|comparison...], "errors": [...], "meta": {...}}`. **`stances` is not verifier input.** Editorial stances — "the fastest path", "the recommended approach", "unlike Terraform" — have no external ground truth, so `verify-claims.py` never receives them and no trail record exists for one; they render verdict-free under ⚠️ as *Editorial stances introduced by this PR* (`docs-review:references:output-format`), and a fix or glow-up lane never banks one as work. Verify only `claims`.
...
The canonical worked-example set — composite/split, implicit comparison, quantitative, negative, the third-party-attribution flip, and the hard run-to-run-disagreement shapes — lives in `docs-review:references:claim-extraction` §Worked examples (14 cases). Don't maintain a parallel copy here; the in-review fallback path reads that file.
...
Five hard rules for every verifier, in-review or pre-step. Each exists because violating it produced false `contradicted` verdicts in the 2026-07 ledger re-adjudication (17 of 22 contradicted verdicts were false; most traced to these):
...
1. **Same-site pages are never ground truth.** A pulumi.com or registry page may corroborate, but never by itself contradicts other Pulumi content — two Pulumi pages disagreeing is an internal inconsistency (both suspect), resolved against product source via `gh`/reads; sibling-consistency claims verdict as `mismatch`. If code can't settle it, `unverifiable`. Exception: *auto-generated* reference pages (CLI command pages under `content/docs/iac/cli/commands/`, API/registry reference generated from schemas) are transcriptions of product source, not editorial content — they carry product-source authority, though quoting the underlying source directly is still stronger evidence. On the routed path this is enforced, not just prompted: when every source a `contradicted` verdict cites is an editorial pulumi.com page (the generated-reference carve-out and a URL the claim itself names are exempt), `verify-claims.py` runs one re-check in the pass1 lane against product source. An independently sourced answer stands; otherwise the verdict is downgraded to `unverifiable` and stamped `source_discipline_gate: "same-site-only"`.
...
1. **The content under review never verifies — or contradicts — itself.** The reviewed file restating a claim is not evidence for it, and the *live published copy* of that file (`https://www.pulumi.com/<its path>`) is not evidence against it: the live page shows the pre-change text, so it disagrees with every value a PR changes. That disagreement is a useful change detector and a worthless judge — it fired falsely on intentional version refreshes and correctly on bot rewrites that weakened a true fact, and only an independent source tells the two apart. On the routed path this is enforced, not just prompted: a `contradicted` or `mismatch` citing the reviewed page's own URL with no independent source alongside (another host, a `gh` command, a repo path to a different file) gets one re-check in the pass1 lane, told the live page is not evidence either way. An independently sourced answer stands, whichever way it goes; otherwise the verdict is downgraded to `unverifiable`, stamped `source_discipline_gate: "self-reference"`, and keeps the original reasoning in `evidence`. A `contradicted` whose only source is the reviewed file's own repo path gets the same treatment (`source_discipline_gate: "own-file-only"`): it means the extracted claim and the page disagree, which is an extraction misreading (`not-a-claim`) or the page contradicting itself (`mismatch`), not a fact about the world. The mirror image is enforced as well: a `verified` whose only source is the reviewed file's repo path confirms nothing. At high or medium confidence it gets the same one re-check, which may land on `not-a-claim` when the claim only describes the page itself, and must otherwise check the claim independently — for a version, against the latest release, since a pin that exists can still be stale. A second `verified` resting on the reviewed file alone is discarded, and an unsettled re-check is downgraded to `unverifiable` with `source_discipline_gate: "own-file-only"`. **Treat any gated verdict as an author question ("what is the source for the changed value?") — never promote it to 🚨 Outstanding.** Apply the same rule when verifying in-review.
