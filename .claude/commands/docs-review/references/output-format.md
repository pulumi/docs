...
- **⚠️ Low-confidence** is for findings where the reviewer is <80% sure *or* where the finding is "worth human attention but not blocking" (e.g., infra risk flags per [`infra.md`](infra.md)). Don't pad with hedging on findings you're confident in.
...
- **✅ Resolved** lists findings from the previous review that no longer appear. Used by [`update.md`](update.md) to give the author signal that their fixes landed.
...
- Secrets, credentials, or tokens present in the diff (always 🚨; see [`infra.md`](infra.md) §Secret handling).
