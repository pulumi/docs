...
**Cite borrowed wording.** If the replacement text takes a clause from another page (a sibling guide, the page a cross-sibling check compared against), say so in the finding -- file and lines, "per `any-terraform-provider.md` L23–24" -- and treat the sibling as a source to be checked, not as ground truth. See `docs-review:references:output-format` §Provenance on suggested rewrites.
...
- title length, meta description length, `meta_image` `.png` extension
