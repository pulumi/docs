...
def added_lines_in_content(patch: str) -> list[str]:
    """Return `+`-prefixed body lines from content/(docs|blog|tutorials|resources)/**/*.md only.
    ...
    for raw in patch.splitlines():
        ...
        if m:
            ...
            in_content = bool(re.match(r"^content/(docs|blog|tutorials|resources)/.*\.md$", current_file))
            continue
