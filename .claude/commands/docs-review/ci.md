...
- the `#### Style suggestions` block (advisory Vale findings from `.vale-findings.json`, always collapsed, uncounted) plus any `[style-blocker]` bullets in 🚨 Outstanding (blocker-tier Vale findings, counted — never delete or demote these);
- the empty 💡 / ✅ forms;
