...
- **Build, test, and infrastructure changes**:
  ...
  - **For infrastructure changes** (infrastructure/, package.json, webpack config, Lambda@Edge, CloudFront): Review the [Infrastructure Change Review](../../../BUILD-AND-DEPLOY.md#infrastructure-change-review) section of BUILD-AND-DEPLOY.md for common risk areas. **Your role is to identify and notify the user of potential risks**, especially: Lambda@Edge bundling issues (ESM/CommonJS, webpack config changes), large dependency update batches, CloudFront distribution changes, and high-risk deployments. Flag these issues clearly so the human reviewer can verify testing and validation.
- **Images and assets**:
