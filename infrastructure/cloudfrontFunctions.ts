...
// front matter in content/): the homepage, /resources/ (whose pre-rebrand
// articles stay pinned at /what-is/ URLs, so both prefixes emit artifacts),
// /product/, and /pricing/. The bare /what-is/ landing is an alias redirect
// with no artifact, but per-article /what-is/<slug>/ paths still emit index.md.
...
const marketingMarkdownNegotiationFunction = new aws.cloudfront.Function("marketing-markdown-negotiation", {
    ...
    comment: "Serves index.md for the homepage, /resources/ (and pinned /what-is/ article URLs), /product/, and /pricing/ via Accept: text/markdown or .md URL suffix.",
});
