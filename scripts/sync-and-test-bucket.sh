...
# The current destination bucket that houses the mono build will eventually only host a build 
# generated by pulumi-hugo. If this is not the testing environment, use the same bucket we
# currently have as the destination bucket as this is the current functional equivalent,
# but in the testing flow, use the bucket from the pulumi-hugo build.
pulumi_hugo_bucket="$destination_bucket"
if [[ "$DEPLOYMENT_ENVIRONMENT" == "testing" ]]; then
    # this is the bucket that gets managed by the pulumi-hugo repo. set it to that bucket in
    # in the metadata file, so there should be no cloudfront origin diff for this bucket when
    # deployments run from this repo.
    pulumi_hugo_bucket=$(pulumi -C infrastructure stack output originS3BucketName)
fi
...
printf "$metadata" "$(current_time_in_ms)" "$(git_sha)" "$pulumi_hugo_bucket" "$destination_bucket" "$s3_website_url" > "$metadata_file"
