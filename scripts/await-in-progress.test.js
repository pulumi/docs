...
function run(overrides = {}) {
    return {
        id: 1,
        status: "in_progress",
        html_url: "https://github.com/pulumi/docs/actions/runs/1",
...
test("keeps a long-running but still-possible run", () => {
    // Just inside the timeout: a real deploy this slow would be pathological,
    // but it could still be holding the stack, so it must not be skipped.
    const kept = dropStaleRuns([run({ run_started_at: ago(maxRunAgeMs - HOUR) })], NOW, () => {});
    assert.equal(kept.length, 1, "a run inside the job timeout must still be waited on");
});
