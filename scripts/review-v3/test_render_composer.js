...
// --stamp flags where act.py expects one, refresh / re-run not counting as
// decisions, a card that goes out looking exactly like one nobody pressed,
// and the interactive handoff leaking into the --act command. Each is a full
// click sequence, not a unit of the script.
"use strict";
const fs = require("fs");
...
const prs = JSON.parse(process.argv[3]);   // {stamp:[bot, human], judge, route:[a,b], refresh, rerun, handfix, team}
...
// 8 rows carry a decision button; the two stamp rows arrive decided.
expect("progress counts every row with a decision to make", progress(), "2 of 8 decisions made");
...
expect("route rows count as decided", progress(), "4 of 8 decisions made");
...
expect("clear resets progress", progress(), "0 of 8 decisions made");
...
expect("refresh and re-run count as decisions", progress(), "2 of 8 decisions made");
expect("their cards light", card("--refresh").classList.contains("sel") && card("--rerun").classList.contains("sel"), true);

// 7. A card is out when one of its rows disagrees -- which, painted the same
// as a card nobody had pressed, read as "this button does nothing". It is
// partly lit now, and says how many of its rows are still with it.
clear();
const stampCard = card("--stamp");
click(stampCard);
expect("stamp card lit from both its rows", stampCard.classList.contains("sel"), true);
expect("and its tally reads full", stampCard.querySelector(".cnt").textContent, `${prs.stamp.length}/${prs.stamp.length}`);
expect("a full card is not partial", stampCard.classList.contains("part"), false);
click(btn(human, `--stamp ${human}:merge`));           // a contrary decision on one of its rows
expect("the card goes out", stampCard.classList.contains("sel"), false);
expect("but reads as partial, not untouched", stampCard.classList.contains("part"), true);
expect("and its tally says how many rows are still with it", stampCard.querySelector(".cnt").textContent, "1/2");
expect("the row's own choice is what composes", cmd(), `$ /pr-review --act --stamp ${bot},${human}:merge`);
click(stampCard);
expect("pressing a partial card takes every row back", stampCard.classList.contains("sel"), true);
expect("and it is no longer partial", stampCard.classList.contains("part"), false);
expect("with the card's fragment on every row", cmd(), `$ /pr-review --act --stamp ${bot},${human}`);

// 8. "fix it yourself" is a run, not a write: its own line, never --act.
clear();
...
expect("a handoff is not a decision", progress(), "0 of 8 decisions made");
