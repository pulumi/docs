...
const eventRawData = fs.readFileSync("./data/events.toml");
const eventData = toml.parse(eventRawData).events;
...
<a class="btn" href="${this.registrationUrl}">Register Here</a>
...
Thank you for checking out the Infrastructure as Code Workshop! More labs are on their way. Please [file an issue](https://github.com/pulumi/infrastructure-as-code-workshop/issues/new) if there are topics you'd like to see covered in the future.
    ...
    writeFile() {
        const fileString = this.generateFrontMatter();
        fs.writeFileSync(`./content/events/${this.urlSlug}.md`, fileString);
    }
}

for (let i = 0; i < eventData.length; i++) {
    const event = eventData[i];
    const file = new FileToWrite(event);
    file.writeFile();
}
