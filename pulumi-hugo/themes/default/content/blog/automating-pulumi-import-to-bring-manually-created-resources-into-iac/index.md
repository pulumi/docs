...
Bringing resources under IaC (and Pulumi specifically) gives an organization an immediate return on investment. Note that while we list some benefits specific to the Pulumi Service, Pulumi supports a number of backends for both [state management](/docs/intro/concepts/state/#deciding-on-a-state-backend) and [secrets management](/docs/intro/concepts/secrets/#available-encryption-providers) and the Pulumi Service is not required in order to adopt Pulumi:
