...
CI/CD. In this second part we will go in-depth on how to securely provide AWS credentials to a
3rd party, and introduce a Pulumi program to automate rotating access keys.
...
In the [first post]({{< relref "/blog/managing-aws-credentials-on-cicd-part-1" >}}) in our series, we created a
dedicated IAM User who will be used to perform updates to AWS resources within your CI/CD system. The next step
is to pass the AWS access keys for that user to your CI/CD system.
...
obtain those keys and do something nefarious. Even if your CI/CD service takes great care to
...
However, there are plenty of reasons to use a hosted service for performing your CI/CD as well. Such as
developer productivity, to ease of use, performance, or simply because you don't want to recreate your
existing deployment workflows.

Ultimately, it is important to just understand the risks involved and how to mitigate them.

## Properly Storing Credentials

If you do choose to provide your CI/CD with credentials, the most important thing is to mark them
as secret.

Your CI/CD provider likely has built-in support for handling "secure variables" or other sensitive
information. (As opposed to general, configuration data or environment variables.)

Typically these are user-supplied configuration values that are write-only and only accessible by
the CI/CD worker jobs at runtime. For example, Travis CI has
[encrypted environment variables](https://circleci.com/docs/2.0/env-vars/#overview) or
[secure environment variables](https://circleci.com/docs/2.0/env-vars/#overview) in CircleCI.

Please consult your CI/CD provider's documentation for how to properly pass and store sensitive
information.

### Why not use a "secrets manager" service? {#using-a-secrets-service}

A common question comes up when discussing how to securely pass access keys to a CI/CD system:
...
In other words, if you choose _not_ to trust your CI/CD system with this data, can you
_instead_ trust some other system dedicated for securely storing and retireving sensitive information?
...
available to the CI/CD job when it starts (and stored via the CI/CD provider). Instead, your CI/CD
job would obtain credentials from the "secrets manager" only when they are needed.

There are some advantages to this approach, such providing a clear audit trail for access, and more
control over the distribution of any sensitive information.

However, whatever secrets provider system you use needs to be presented with some form of credentials, too.
And _those_ credentials need to be available to your CI/CD environment. So using a secrets manager
leaves you in the same place you started: needing to provide sensitive data to your CI/CD provider.

Also, by adding a dependency on a secrets manager, you are introducing additional risks. That secrets
Not only do you need to be even more security-concious about that secrets manager, but it also needs to
be highly available too. (Since any type of outage for that service would mean that you would be unable
to perform deployments!)

So it does not seem that using a secrets manager to dole out AWS credentials to your CI/CD system
is a good practice to follow. Or at the very least, makes some tradeoffs without fundmanentally making
your approach to CI/CD any more or less secure.

Instead, it is better to focus on other ways to reduce the risk if those AWS credentials do get exposed.

## Automate Rotating and Revoking AWS Credentials {#automating-key-rotation}

When providing credentials to a 3rd party, rather than hoping it will be 100% secure
...
The AWS security blog describes how to
[rotate access keys for IAM Users](https://aws.amazon.com/blogs/security/how-to-rotate-access-keys-for-iam-users/),
...
That all sounds simple enough, but it's certainly tedious. And if you need to repeat the process
across dozens if not hundreds of different CI/CD pipelines, you will need a better solution.
Fortunately, Pulumi provides an excellent and extensible way for writing a serverless programs to
automate rotating credentials.

The next few sections will describe a simple infrastructure app for automating AWS IAM credential
rotation. But you can see the full application on GitHub over at
[chrsmith/pulumi-aws-travis-cicd-demo](https://github.com/chrsmith/pulumi-aws-travis-cicd-demo).

### AWS Credential Rotator 9000

The affectionally titled _AWS Credential Rotator 9000_ is a simple, serverless Pulumi application
for rotating AWS access keys.

It creates an AWS lambda function that will be triggered on a regular schedule, e.g. every
30 minutes, and performs the next step in the sequence for rotating access keys as outlined
above.

First it will create a new access key, and push the new value out. On the next iteration
it will mark the older access key as "inactive". next iteration after that, the "inactive"
key will be deleted. And so on.

### Periodically Invoking an AWS Lambda

The heart of the application is triggering some code to execute on a fixed interval. But
thankfully this is super-easy to do using Pulumi, since it allows you to seemlessly blend
your "cloud infrastructure" with "code" in a natural way. The user guide for Pulumi Crosswalk
for AWS has more information on [serverless eventing]({{< ref "/docs/guides/crosswalk/aws/lambda" >}})
if you would like to learn more.

The following snippet is the core part of the credential rotator app. We first define a function
to handle the logic of key rotation, in `rotateIAMUserKeys`. We then create an AWS Lambda resource
named `lambda`. Finally, the `triggerSchedule` resource will invoke our lambda on a fixed
schedule. Thereby ensuring that the key rotation process goes on indefinitely. (You can see the
full code
...
When writing reusable infrastructure components in Pulumi however, it is helpful to
organize things into a [custom resource]({{< ref "/docs/intro/concepts/programming-model#custom-resources" >}}).

For example, we can bundle together the AWS Lambda, CloudWatcn schedule, and the associated IAM policies
into a single conceptual resource `AccessKeyRotator`. This allows for the code to be reused.
...
With the mechanics of updating an AWS access key out of the way, the next step is to
actually notify dependent systems what the new access key should be. The key rotator
app has a `CredentialPusher` abstraction to provide a pluggable way for you to
send credentials to where they need to be. The code on GitHub only supports updating
a Travis CI project (See [`credential-pusher-travis.ts`](https://github.com/chrsmith/pulumi-aws-travis-cicd-demo/blob/master/infrastructure/key-rotator/credential-pusher-travis.ts).), but could easily be extended to support other CI/CD
systems.

The following snippet shows how the `CredentialPusher` is configured in the key
rotator app. We load a Travis CI access token from the
[Pulumi stack's configuration](https://www.pulumi.com/docs/intro/concepts/config/),
and hard-code the specific set of projects and encrypted environment variables to
store the new access key in.
...
To demonstrate access keys being rotated, we can just look at the log files generated from AWS Lambda. These can
be accessed from the command-line using the [`pulumi logs`]({{< ref "/docs/reference/cli/pulumi_logs" >}}) command.
...
As you can see, the AWS credentials for the IAM User are automatically updated every few hours.
And whenever a new access key is created, the value is pushed out to the impacted Travis CI projects
automatically.

## Wrapping Up

In this post we covered some of the things to consider when providing AWS credentials to
your CI/CD system. (In short, be cautious and follow best practices.)

We then showed how relatively easy it is to stand up a Pulumi infrastructure application
for automatically rotating AWS access keys and updating your CI/CD system. By rotating
credentials you can limit the impact if the value does inadvertenly get disclosed.

So at this point we've now given access keys to a CI/CD system for a low-privledge AWS
IAM User account. But we still can't _do_ anything with it. The IAM User whose credentials
we have given to the CI/CD system doesn't have access to your production AWS account.

In the next post, we'll go into the details about IAM Roles and they can be used to
securely, and temporarily gain access to additional resources. This is how our low-privledge
IAM User will be able to get the access needed to update production data for your
CI/CD pipelines.
