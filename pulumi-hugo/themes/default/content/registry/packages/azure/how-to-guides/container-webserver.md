...
1. [Configure Azure credentials]({{< relref "/registry/packages/azure/installation-configuration" >}})
...
1. Open `index.js` and replace the contents with the following:
    ...
    This example uses the [@pulumi/azure]({{< relref "/registry/packages/azure" >}}) package to create and manage two Azure resources including: an [azure.core.ResourceGroup]({{< relref "/registry/packages/azure/api-docs/core/resourcegroup" >}}) which will contain the ACI instance and [azure.containerservice.Group]({{< relref "/registry/packages/azure/api-docs/containerservice/group" >}}) which will run an `nginx` Docker container.
...
1. To view the provisioned resources on the command line, run `pulumi stack`. You'll also see a [stack output]({{< relref "/docs/intro/concepts/stack#output" >}}) corresponding to the private IP address of the nginx container we've created.
