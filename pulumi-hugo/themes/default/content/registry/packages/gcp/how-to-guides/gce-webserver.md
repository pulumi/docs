...
1. Open `index.js` and replace the contents with the following:
    ...
    This example uses the [@pulumi/gcp]({{< relref "/registry/packages/gcp" >}}) package to create and manage three Google Cloud resources: a [gcp.compute.Network]({{< relref "/registry/packages/gcp/api-docs/compute/network" >}}) in which the virtual machine will run, a [gcp.compute.Firewall]({{< relref "/registry/packages/gcp/api-docs/compute/firewall" >}}) which allows access for incoming SSH access, and a [gcp.compute.Instance]({{< relref "/registry/packages/gcp/api-docs/compute/instance" >}}) which is created inside the network from the Debian 9 base image.
