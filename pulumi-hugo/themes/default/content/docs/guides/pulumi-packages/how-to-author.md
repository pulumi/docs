...
{{% notes %}}
API docs for your package will be automatically generated by automated pipelines at the time of building the registry site. You do not need to take any action to generate API docs other than make sure your package repository has the right `schema.json` (or `.yaml`) and that the metadata file for your package points to the correct schema file.
{{% /notes %}}
