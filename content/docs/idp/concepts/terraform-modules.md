...
`terraform login app.pulumi.com` produces the bearer token used by every HashiCorp-protocol service Pulumi Cloud exposes (state backend, module registry). The credential lands in `~/.terraform.d/credentials.tfrc.json`.

Run it once per workstation:

```bash
terraform login app.pulumi.com
```
...
Module names containing `_` are rejected at publish; rename to `-` before pushing.
...
```hcl
module "vpc" {
  source  = "app.pulumi.com/<namespace>/<name>/<system>"
  version = "1.2.3"
}
```
...
module "private_subnet" {
  source  = "app.pulumi.com/<namespace>/<name>/<system>//modules/<submodule>"
  version = "1.2.3"
}
```
