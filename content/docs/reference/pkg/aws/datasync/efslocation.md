...
```python
def get(resource_name, id, opts=None, arn=None, ec2_<wbr>config=None, efs_<wbr>file_<wbr>system_<wbr>arn=None, subdirectory=None, tags=None, uri=None)
```

```go
func GetEfsLocation(ctx *pulumi.Context, name string, id pulumi.IDInput, state *EfsLocationState, opts ...pulumi.ResourceOption) (*Bucket, error)
```
