...
```python
def LustreFileSystem(resource_name, id, opts=None, export_<wbr>path=None, import_<wbr>path=None, imported_<wbr>file_<wbr>chunk_<wbr>size=None, security_<wbr>group_<wbr>ids=None, storage_<wbr>capacity=None, subnet_<wbr>ids=None, tags=None, weekly_<wbr>maintenance_<wbr>start_<wbr>time=None, __props__=None)
```
