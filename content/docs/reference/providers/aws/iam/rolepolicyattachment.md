...
```python
def RolePolicyAttachment(resource_name, id, opts=None, policy_<wbr>arn=None, role=None, __props__=None)
```
...
<table class="ml-6">
 ...
 {{% /md %}}
        ...
        <tr>
            ...
            <td class="align-top">{{% md %}} The role the policy should be applied to
 {{% /md %}}
...
```python
def get(resource_name, id, opts=None, policy_<wbr>arn=None, role=None)
```

```go
func GetRolePolicyAttachment(ctx *pulumi.Context, name string, id pulumi.IDInput, state *RolePolicyAttachmentState, opts ...pulumi.ResourceOption) (*Bucket, error)
```
