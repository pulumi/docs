...
```python
def DataSource(resource_name, id, opts=None, api_<wbr>id=None, description=None, dynamodb_<wbr>config=None, elasticsearch_<wbr>config=None, http_<wbr>config=None, lambda_<wbr>config=None, name=None, service_<wbr>role_<wbr>arn=None, type=None, __props__=None)
```
