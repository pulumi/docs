...
```python
def FileSystem(resource_name, id, opts=None, creation_<wbr>token=None, encrypted=None, kms_<wbr>key_<wbr>id=None, lifecycle_<wbr>policy=None, performance_<wbr>mode=None, provisioned_<wbr>throughput_<wbr>in_<wbr>mibps=None, tags=None, throughput_<wbr>mode=None, __props__=None)
```
