...
```python
def DomainPolicy(resource_name, id, opts=None, access_<wbr>policies=None, domain_<wbr>name=None, __props__=None)
```
...
```python
def get(resource_name, id, opts=None, access_<wbr>policies=None, domain_<wbr>name=None)
```

```go
func GetDomainPolicy(ctx *pulumi.Context, name string, id pulumi.IDInput, state *DomainPolicyState, opts ...pulumi.ResourceOption) (*Bucket, error)
```
