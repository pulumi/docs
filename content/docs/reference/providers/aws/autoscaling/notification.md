...
```python
def Notification(resource_name, id, opts=None, group_<wbr>names=None, notifications=None, topic_<wbr>arn=None, __props__=None)
```
...
```python
def get(resource_name, id, opts=None, group_<wbr>names=None, notifications=None, topic_<wbr>arn=None)
```

```go
func GetNotification(ctx *pulumi.Context, name string, id pulumi.IDInput, state *NotificationState, opts ...pulumi.ResourceOption) (*Bucket, error)
```
