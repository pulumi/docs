...
```python
def Key(resource_name, id, opts=None, customer_<wbr>master_<wbr>key_<wbr>spec=None, deletion_<wbr>window_<wbr>in_<wbr>days=None, description=None, enable_<wbr>key_<wbr>rotation=None, is_<wbr>enabled=None, key_<wbr>usage=None, policy=None, tags=None, __props__=None)
```
