...
```python
def Fleet(resource_name, id, opts=None, audit_<wbr>stream_<wbr>arn=None, device_<wbr>ca_<wbr>certificate=None, display_<wbr>name=None, identity_<wbr>provider=None, name=None, network=None, optimize_<wbr>for_<wbr>end_<wbr>user_<wbr>location=None, __props__=None)
```
