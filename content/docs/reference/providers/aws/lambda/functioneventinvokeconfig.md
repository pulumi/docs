...
```python
def FunctionEventInvokeConfig(resource_name, id, opts=None, destination_<wbr>config=None, function_<wbr>name=None, maximum_<wbr>event_<wbr>age_<wbr>in_<wbr>seconds=None, maximum_<wbr>retry_<wbr>attempts=None, qualifier=None, __props__=None)
```
