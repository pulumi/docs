...
```python
def get(resource_name, id, opts=None, api_<wbr>stages=None, arn=None, description=None, name=None, product_<wbr>code=None, quota_<wbr>settings=None, tags=None, throttle_<wbr>settings=None)
```

```go
func GetUsagePlan(ctx *pulumi.Context, name string, id pulumi.IDInput, state *UsagePlanState, opts ...pulumi.ResourceOption) (*Bucket, error)
```
