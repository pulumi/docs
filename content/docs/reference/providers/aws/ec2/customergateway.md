...
```python
def CustomerGateway(resource_name, id, opts=None, bgp_<wbr>asn=None, ip_<wbr>address=None, tags=None, type=None, __props__=None)
```
...
```python
def get(resource_name, id, opts=None, bgp_<wbr>asn=None, ip_<wbr>address=None, tags=None, type=None)
```

```go
func GetCustomerGateway(ctx *pulumi.Context, name string, id pulumi.IDInput, state *CustomerGatewayState, opts ...pulumi.ResourceOption) (*Bucket, error)
```
