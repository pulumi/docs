...
```python
def AmiLaunchPermission(resource_name, id, opts=None, account_<wbr>id=None, image_<wbr>id=None, __props__=None)
```
...
```python
def get(resource_name, id, opts=None, account_<wbr>id=None, image_<wbr>id=None)
```

```go
func GetAmiLaunchPermission(ctx *pulumi.Context, name string, id pulumi.IDInput, state *AmiLaunchPermissionState, opts ...pulumi.ResourceOption) (*Bucket, error)
```
