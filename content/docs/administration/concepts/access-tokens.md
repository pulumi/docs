...
To remove the policy, set the value to 0 (or clear the field) and save. Policy changes are recorded in the organization's [audit logs](/docs/administration/concepts/audit-logs/). While a policy is active, the **Access Tokens** tab shows a banner with the current cap and an **Edit policy** shortcut to this setting.
