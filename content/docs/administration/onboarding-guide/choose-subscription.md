...
{{% notes type="info" %}}
Both billing options include detailed usage insights through the Billing & usage page in your organization settings. Track IaC resources, deployment minutes, ESC secrets, and download usage history. You can also set [Neo usage limits](/docs/ai/usage-limits/) to cap spend on Pulumi Neo. Only organization administrators and designated [billing administrators](/docs/administration/organizations-teams/billing-managers/) can access these pages. You'll also receive monthly usage reports via email.
{{% /notes %}}
