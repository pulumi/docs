...
menu:
    administration:
        ...
        weight: 6
        identifier: administration-guides-least-privilege
aliases:
...
- /docs/administration/security-compliance/least-privilege/
---
...
- **Access to Pulumi Cloud itself**, governed by [role-based access control (RBAC)](/docs/administration/concepts/rbac/), which decides who can read state, run updates, and unlock secrets.
...
Grant elevated access with [permission sets](/docs/administration/concepts/rbac/permission-sets/#stack-permission-sets), assigned through roles and teams rather than to individuals:
...
Organize people into [teams](/docs/administration/concepts/rbac/teams/) and give each team the least access its work requires. A team can hold one or more roles and be granted access to specific stacks. You can also grant a team access to a stack when you create it:
...
Grant environment access with [environment permission sets](/docs/administration/concepts/rbac/permission-sets/#environment-permission-sets):
...
When you grant a team environment access, Pulumi Cloud labels these Environment reader, opener, editor, and admin. The [teams reference](/docs/administration/concepts/rbac/teams/) has the full mapping.
...
Where OIDC is not an option, you can fall back to a stored [Pulumi access token](/docs/administration/concepts/access-tokens/). This is the least preferable choice, because a long-lived token in a CI secret is exactly the standing credential OIDC is designed to remove. If you must use one, prefer a team or organization token over a personal one, give it the shortest workable expiration, and rotate it regularly.

### Hand off ownership of stacks your automation creates

Whichever credential a pipeline uses, creating a stack leaves a permanent mark on the identity that created it. The creator receives an automatic [creator grant](/docs/administration/concepts/rbac/#creator-grants) of Stack Admin on that stack, and because every grant in Pulumi Cloud is additive, no role, organization-wide setting, or tag-based rule takes it back. The grant belongs to the organization or team the token authenticates as, so it outlives the token itself, including a short-lived OIDC-issued one.
...
1. Review [audit logs](/docs/administration/concepts/audit-logs/) for access and deployment activity.
