...
{{< notes type="info" >}}
Pulumi ESC should be the preferred approach over the Deployments OIDC integrations since it's more portable and easier to set up. For ESC OIDC setup instructions, see [Configuring OIDC for AWS in ESC](/docs/esc/environments/configuring-oidc/aws/).
{{< /notes >}}
...
For more granular access control, edit the trust policy of your IAM role with [Token claims](/docs/pulumi-cloud/deployments/oidc/#custom-claims) for deployments. The `sub` claim can be customized as shown below.
