...
- **Azure Resource Manager**: the project and the stack's scope are both drawn from the resource group's name, so a `payments-network` resource group scanned under the `prod` cloud account becomes project `payments-network`, stack `prod-payments-network` (no suffix here, since `network` isn't a recognized environment token).
