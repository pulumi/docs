...
1. Replace the contents of `index.js` with the following:
    ...
    - **Containers**. Uses [cloud.Task]({{< prelref "/docs/reference/pkg/nodejs/pulumi/cloud#Task" >}}), which is a high-level, convenient component for working with containers. The component automatically provisions a container registry instance in ECR, runs a Docker build, and saves the Docker image to the provisioned ECR instance. It also defines an ECS task and configures it to use the built image.
