...
values:
  ...
  # variables when using `pulumi esc open --format=shell`, `pulumi esc run`, or `pulumi up/preview/refresh/destroy`
  ...
  # when using 'pulumi esc open --format=shell', 'pulumi esc run', or `pulumi up/preview/refresh/destroy`
  files:
