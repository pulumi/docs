...
- [Pulumi CLI](https://www.pulumi.com/docs/get-started/install/) version 3.132.0 or later
...
To create a new ESC environment:

1. In the ESC Explorer, locate your **organization** and **project**
2. Click the **plus (+) icon** next to the project

The extension will open up an editor with an example environment definition you can then edit.

![Pulumi ESC Explorer - create an environment](/docs/esc/assets/esc-vscode-add-env.png)
...
When you `open` an environment in Pulumi ESC, you're instructing the ESC to process the environment definition, decrypt any secrets, and resolve all configuration values. This allows you to view the fully resolved environment with all variables expanded and secrets decrypted in your chosen output format.
