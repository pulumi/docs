...
This property is also used by [Pulumi policy packs](/docs/discovery-governance/concepts/policy-as-code/policy-packs/). When an ESC environment is attached to a policy pack in a policy group, `environmentVariables` are injected into the policy runtime as environment variables.
