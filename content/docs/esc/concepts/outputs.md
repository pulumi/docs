...
This property is also used by [Pulumi policy packs](/docs/discovery-governance/concepts/policy-packs/). When an ESC environment is attached to a policy pack in a policy group, `environmentVariables` are injected into the policy runtime as environment variables.
...
The `policyConfig` reserved property contains values that should be exported as configuration for [Pulumi policy packs](/docs/discovery-governance/concepts/policy-packs/). When an ESC environment is attached to a policy pack in a policy group, the values under `policyConfig` are made available to the policy pack at runtime.
