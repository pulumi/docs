...
Once you've created the example environment, you can now use the Terraform CLI with `pulumi esc run` to invoke Terraform with credentials and configuration:

```bash
$ pulumi esc run <your-project-name>/<your-env-name> -i -- terraform apply
```

This will open your ESC environment, export its environment variables, and then invoke `terraform apply` with those environment variables. The `-i` instructs `pulumi esc` to run in interactive mode, which is necessary for programs like the Terraform CLI that expect user input.
...
```bash
$ pulumi esc run <your-project-name>/<your-env-name> -i -- sh -c 'mv $TFVARS $TFVARS.json && terraform apply -var-file=$TFVARS.json'
```
