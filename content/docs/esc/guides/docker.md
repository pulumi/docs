...
```bash
$ pulumi env run <your-project-name>/<your-environment-name> -- docker run --rm -t -e ESC_ORG -e ESC_HELLO_USER alpine env
```
...
```bash
$ pulumi env open pulumi/examples/docker-env-test --format shell
...
```bash
$ pulumi env run -i <your-project-name>/<your-environment-name> -- sh -c 'docker run --rm -t --env-file=$DOCKER_ENVFILE alpine env'
```
...
```bash
$ pulumi env run <your-project-name>/<your-environment-name> -- sh -c 'echo ${docker.password} | docker login --username ${docker.username} --password-stdin ${docker.registry}'
