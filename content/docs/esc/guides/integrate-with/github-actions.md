...
Before you can use OIDC, you must configure Pulumi Cloud to trust GitHub Actions as an OIDC issuer. Follow the [Configuring OpenID Connect for GitHub](/docs/administration/guides/oidc-issuers/github/) guide to complete this one-time setup in your Pulumi organization.
...
As an alternative to OIDC, you can authenticate with a long-lived [Pulumi access token](/docs/administration/concepts/access-tokens/). Store the token as a repository secret and set it as the `PULUMI_ACCESS_TOKEN` environment variable in your workflow:
