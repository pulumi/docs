...
Encrypted [secrets]({{< prelref "/docs/intro/concepts/programming-model#secrets" >}}) are decrypted during previews and updates. Any policy that is run against a stack can access the values in plaintext. It is up to you to treat these values sensitively and only run policies that you trust.
