...
> [Pulumi Crosswalk for AWS EKS]({{< prelref "eks" >}}) for more information about using EKS.
...
options described in the [Pulumi Crosswalk for ELB documentation]({{< prelref "elb" >}}) can be used with our ECS service.
...
described in [Pulumi Crosswalk for AWS VPC]({{< prelref "vpc" >}}). Then we simply pass that
...
Notice that we are using a method from a different package, [`aws.ecs.Cluster.get`]({{< prelref "/docs/reference/pkg/aws/ecs/cluster#look-up" >}}), to look up our existing cluster by its ID and then creating an `awsx.ecs.Cluster` out of it. The former is the raw resource description, while the latter is the object type required to work with the Pulumi Crosswalk for AWS ECS APIs.
...
For example, [`fromPath`]({{< prelref "/docs/reference/pkg/nodejs/pulumi/awsx/ecs#Image-fromPath" >}}) will run a `docker build` in that path, push the result up to the ECR repository that specified in the first argument, and then pass
...
For more control over how the Docker image is built and published, you can use [`fromDockerBuild`]({{< prelref "/docs/reference/pkg/nodejs/pulumi/awsx/ecs#Image-fromDockerBuild" >}}). This allows you
...
runs in the container within your Pulumi application directly, much like [magic functions for Lambda]({{< prelref "lambda" >}}):
...
For more information about using ECR, refer to [Pulumi Crosswalk for AWS ECR]({{< prelref "ecr" >}}).
