...
aliases:
  ...
  - /docs/discovery-governance/policy/policy-packs/authoring/
---
...
Policies can be written in TypeScript/JavaScript (Node.js), Python, or OPA (Rego) and can be applied to Pulumi stacks written in any language. Learn more about [language support for policies](/docs/discovery-governance/concepts/policy-as-code/#languages).
...
1. Replace the generated policy in `index.ts` with this example, which demonstrates a clearer pattern for organizational policy enforcement:
    ...
    > For more information on all available fields, see [policy fields](/docs/discovery-governance/reference/policy-fields/).
...
- The `remediationSteps` metadata field (see [policy fields](/docs/discovery-governance/reference/policy-fields/)) is unrelated to automatic remediation: the field is manual guidance shown to a user for policies that only validate, describing how to fix a violation by hand.
...
1. Run `pulumi preview` again. This time, the policy violation blocks the preview:
    ...
    Policies:
        ❌ policy-pack-typescript@v0.0.1 (local: <path-to-policy-pack>)
            - [mandatory]  rds-storage-encryption  (aws:rds:Instance: my-db)
...
1. Run `pulumi preview` again. This time, the policy violation blocks the preview:
        ...
        Policies:
            ❌ policy-pack-python@v0.0.1 (local: <path-to-policy-pack>)
                - [mandatory]  rds-storage-encryption  (aws:rds:Instance: my-db)
                  RDS instance must have storage encryption enabled (or be tagged 'data-classification=non-production').
...
For a complete list of `PulumiPolicy.yaml` fields, see the [project file reference](/docs/discovery-governance/reference/policy-project-file/).
...
After publishing, your policy pack appears in Pulumi Cloud's policy pack list. Apply it to stacks or cloud accounts using policy groups. See [Get Started with Pulumi Policies](/docs/discovery-governance/get-started/enforce-policy-as-code/) for details.
...
- [Policy as Code overview](/docs/discovery-governance/concepts/policy-as-code/)
- [Policy fields](/docs/discovery-governance/reference/policy-fields/)
- [`pulumi policy` CLI commands](/docs/iac/cli/commands/pulumi_policy/)
