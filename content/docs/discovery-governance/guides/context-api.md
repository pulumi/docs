...
---
The Context API is a read-only Pulumi Cloud API for querying the infrastructure graph. For an introduction to the product, use cases, and access requirements, see the [Context API overview](/docs/discovery-governance/concepts/context-api/).
...
The graph connects nodes like resources (IaC or [Discovered](/docs/discovery-governance/concepts/discovery/)) and stacks through relationships such as dependencies, parent-child links, provider ownership, and stack output consumption.
...
- The organization used for the request has Context API access. `pulumi api` uses the organization for your currently selected stack, or your default organization when no stack is selected. Check the default with `pulumi org get-default` and change it with `pulumi org set-default <your-org>`. If the selected organization does not have access, the API returns `402 Payment Required` and lists any of your other organizations that do.
...
- Resources pending deletion are outside the graph, so a graph count can be lower than a [Resource Search](/docs/discovery-governance/guides/search-resources/) count over an otherwise similar selection.
...
| `query`    | No       | A [Resource Search query](/docs/discovery-governance/guides/search-resources/) string. Valid only for a `resource` anchor and mutually exclusive with `match`. |
