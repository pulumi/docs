...
title: PulumiPolicy.yaml
h1: Policy pack project file (PulumiPolicy.yaml)
menu:
  discovery-governance:
    name: PulumiPolicy.yaml
    parent: dg-reference
    weight: 30
aliases:
  - /docs/insights/policy/policy-packs/pulumi-policy-yaml/
  - /docs/insights/policy/policy-packs/project-file/
  - /docs/discovery-governance/policy/policy-packs/project-file/
---
...
[OPA (Open Policy Agent)](https://github.com/pulumi/pulumi-policy-opa) policy packs let you write policies in [Rego](https://www.openpolicyagent.org/docs/latest/policy-language/) instead of TypeScript or Python. The Pulumi CLI installs the OPA analyzer plugin automatically the first time you run an OPA policy pack. For a walkthrough, see [Write OPA policies](/docs/discovery-governance/guides/write-opa-policies/).

| Name | Required | Description |
| - | - | - |
| `runtime` | required | Must be `opa`. |
| `version` | optional | The version of the policy pack, following [semantic versioning](https://semver.org/). Always set it: without a version, `pulumi preview` output can't identify the pack correctly. |
| `description` | optional | A brief description of the policy pack. |
...
Each version can only be published once. When you publish a new version, update the version number before running `pulumi policy publish`. See [managing policy pack versions](/docs/discovery-governance/guides/write-a-policy-pack/#managing-policy-pack-versions) for details.
