...
[Resource Search](/docs/discovery-governance/guides/search-resources/) filters and groups indexed resources by their properties. Start there when a set of resource records can answer the question.
...
Queries may narrow anchor selection with `scope`, group matching nodes with `aggregate`, and page through larger results. See the [Context API query guide](/docs/discovery-governance/guides/context-api/) for details and examples.
...
Use `pulumi api GraphQuery` to call the API from the CLI, or send a request to the Graph Query endpoint through the [Pulumi Cloud REST API](/docs/reference/cloud-rest-api/). Most people start by [asking a question through an AI agent](/docs/discovery-governance/guides/context-api/#ask-questions-through-an-ai-agent). To see how a selector maps to an API response, [walk through a representative query](/docs/discovery-governance/guides/context-api/#walk-through-a-representative-query).
...
See [Check completeness before acting](/docs/discovery-governance/guides/context-api/#check-completeness-before-acting) for the full interpretation rules and recovery guidance.

## Next steps

- [Ask questions through an AI agent](/docs/discovery-governance/guides/context-api/#ask-questions-through-an-ai-agent).
- [Walk through a representative query](/docs/discovery-governance/guides/context-api/#walk-through-a-representative-query).
- [Learn the query mental model and work through complete examples](/docs/discovery-governance/guides/context-api/).
- [Review the formal selector and response reference](/docs/discovery-governance/guides/context-api/#query-reference).
- [Fetch the deployed schema and agent primer](/docs/discovery-governance/guides/context-api/#get-the-deployed-schema).
