...
- **Air-gapped operation.** Run with no egress to the public internet. Because you install and operate it, the deployment sits inside a compliance boundary you own and assess.
...
1. [Request a Proof of Concept (PoC)](/product/self-hosted/#self-hosted-trial) to get an evaluation license key and a solutions architect to size the install.
1. Follow the [install guide](/docs/administration/self-hosting/install/) — evaluate on a single host with Docker Compose, then deploy to production on AWS, Azure, Google Cloud, or Kubernetes.
