...
New Terraform stacks run plans and applies remotely by default on Pulumi-hosted runners, matching the behavior your team already expects from HCP Terraform and Terraform Enterprise, with full visibility from both your local CLI and the Pulumi Cloud console. Production applies can be gated behind manual approval.
...
* Run [preventive policies](/docs/insights/policy/) against a Terraform plan to block non-compliant resources before they reach your cloud, and keep scanning with audit policies afterward.
...
Pulumi Cloud's private registry also hosts Terraform modules alongside Pulumi packages, so your teams have one place to look for reusable building blocks rather than two. Modules hosted there work from both Pulumi and Terraform programs.
...
Pulumi lets you define infrastructure in the same general-purpose languages your engineering organization already uses: Python, TypeScript, JavaScript, Go, C#, and Java, plus YAML for teams that prefer a markup format and [HCL](/docs/iac/languages-sdks/hcl/) for teams that want to keep the syntax they already know. Choosing a general-purpose language means real loops, conditionals, classes, and functions instead of a DSL's limited expressiveness; the testing frameworks, linters, and IDE tooling (autocomplete, type checking, go-to-definition) your teams already rely on for application code; and dependency management through the same package managers — npm, PyPI, NuGet, Maven, Go modules — your teams use everywhere else. Infrastructure code becomes software, reviewed, tested, and refactored the same way, rather than a separate discipline bolted onto the side of engineering.
...
| Terraform and OpenTofu state | [Pulumi Cloud implements the Terraform remote backend API](/docs/iac/get-started/terraform/terraform-state-backend/) as a drop-in target, with remote plans and applies and manual approval gates, alongside Pulumi's own state | Native, but state and runs are scoped to HCP Terraform workspaces |
| Terraform module reuse | [`pulumi package add terraform-module`](/docs/iac/get-started/terraform/terraform-modules/) pulls existing modules into a Pulumi program in any language, and Pulumi Cloud's private registry hosts them alongside Pulumi packages | Native to Terraform and OpenTofu only |
...
| Policy as code | [Pulumi Policies](/docs/insights/policy/), written in Python, TypeScript, or OPA Rego, enforced on every `pulumi up` and against Terraform plans, as part of the same platform | Sentinel or OPA policy checks, run as a distinct step in the HCP Terraform run pipeline |
...
1. **Reuse what you've built.** Bring your existing [Terraform modules](/docs/iac/get-started/terraform/terraform-modules/) into new Pulumi projects instead of rewriting them.
...
Yes. Pulumi Cloud implements the Terraform remote backend API, so you add a standard `backend "remote"` block and your resource code stays as it is. Plans and applies run remotely on Pulumi-hosted runners by default, with optional manual approval before apply, and the resulting stack gets RBAC, policy enforcement, audit history, and Resource Search like any other Pulumi Cloud stack. See [Using Pulumi Cloud as a Terraform state backend](/docs/iac/get-started/terraform/terraform-state-backend/).
...
Yes, without modifying them. `pulumi package add terraform-module <source> <version> <name>` makes a module available to a Pulumi program in any language, resolving from the Terraform Registry, a private registry, or a local path. Pulumi Cloud's private registry can also host your Terraform modules alongside Pulumi packages, where they remain usable from both Pulumi and Terraform programs. See [Using Terraform modules in Pulumi](/docs/iac/get-started/terraform/terraform-modules/).
