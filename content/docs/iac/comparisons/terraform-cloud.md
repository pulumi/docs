...
| Modularity and reuse | [Component Resources](/docs/iac/concepts/components/) authored in any supported language; [Pulumi Packages](/docs/iac/concepts/packages/) let a component written in one language be consumed from any Pulumi language; language-native package managers (npm, PyPI, NuGet, Maven, Go modules); and the [Pulumi Registry](/registry/) for publicly available packages; plus existing Terraform modules pulled in directly with [`pulumi package add hcl module`](/docs/iac/get-started/terraform/terraform-modules/) and [hosted in Pulumi Cloud's registry](/docs/idp/concepts/terraform-modules/) via an HCP-compatible publish API | Modules published to the Terraform Registry or a private registry, resolvable only from Terraform and OpenTofu configurations |
