...
| Execution model | Local CLI, programmatic via [Automation API](/docs/iac/automation-api/), or remote runs in [Pulumi Deployments](/docs/deployments/) | Centralized Azure-managed service driven through the Azure Portal, Azure CLI (`az deployment`), Azure PowerShell, the Bicep CLI, or the ARM REST API |
