...
AWS CDK, Terraform, and Pulumi all provision cloud infrastructure as code, but they differ in where they run and how an AI coding agent works with them. CDK compiles to CloudFormation and is AWS-only; Terraform uses its own HCL language across many clouds; Pulumi uses general-purpose languages (or HCL) across any cloud, with the shortest feedback loop between the code an agent writes and the errors it reads back.

## Which of CDK, Terraform, and Pulumi can an AI agent write most reliably?

Recent research on LLM-generated infrastructure as code finds that raw syntax is largely a solved problem: a 2026 benchmark comparing CloudFormation, Terraform, and AWS CDK generation found [more than 95% syntactic validity across all three formats](https://arxiv.org/abs/2509.05303), with the real gap in "semantic alignment and handling complex infrastructure patterns." Syntax isn't the differentiator; the verification loop is.
...
This is where the three tools diverge structurally. AWS CDK code passes through [JSII](https://docs.aws.amazon.com/cdk/v2/guide/languages.html) and `cdk synth` into a CloudFormation template before anything is validated against real infrastructure; an agent's errors surface against synthesized logical IDs one translation step removed from the code it wrote. Terraform's `terraform plan` is fast and accurate, but HCL is a domain-specific language with limited abstraction, and tests live in a separate harness ([`terraform test`](https://developer.hashicorp.com/terraform/language/tests)) rather than the language itself. Pulumi programs are written in the same general-purpose language (TypeScript, Python, Go, C#, Java, or HCL) the agent already uses elsewhere in the codebase, so a type checker, linter, and unit tests can catch errors before any cloud call, and `pulumi preview` returns a direct resource-level diff with no template-synthesis step in between.

Pulumi's own benchmark of Claude Opus 4.6 and GPT-5.2-Codex against equivalent Terraform and Pulumi generation tasks found the [total cost of an agent completing a generation-plus-refactor pipeline was 41% lower with Pulumi than Terraform ($0.146 vs. $0.249)](/blog/token-efficiency-vs-cognitive-efficiency-choosing-iac-for-ai-agents/), and that "the difference comes entirely from repair cycles: Pulumi needed zero repairs across both scenarios, while Terraform refactoring triggered self-repair on every run." Fewer translation steps and more verifiers close to the code an agent wrote means fewer repair cycles, which is where the real cost and reliability gap shows up.
...
| --- | --- | --- | --- |
| Languages | TypeScript, Python, Go, C#, Java, YAML, and HCL | HCL (proprietary DSL) | TypeScript, Python, Java, C#, Go (via JSII) |
| Language type | General-purpose, plus a first-class HCL runtime | Domain-specific | General-purpose, compiled to a DSL output |
| Cloud coverage | Any cloud or SaaS via [170+ providers](/registry/) | Any cloud via Terraform/OpenTofu providers | AWS only |
...
| Policy as code | Built-in ([CrossGuard](/docs/insights/policy/)) | Sentinel/OPA (HCP Terraform or separate tooling) | CloudFormation Guard, cdk-nag (separate tooling) |
...
AWS CDK is scoped to AWS: it synthesizes to CloudFormation, and CloudFormation only understands AWS (and a small set of third-party resource types registered with it). There is no supported path to provision Azure, Google Cloud, or Kubernetes resources from a CDK app without stepping outside CDK entirely.

Terraform and Pulumi both provision any cloud through a provider ecosystem. The [Terraform Registry lists 7,010 providers](https://registry.terraform.io/) (measured 2026-08-07), and OpenTofu — the Linux Foundation-stewarded fork created after Terraform's 2023 license change — reports [3,900+ providers and 23,600+ modules](https://opentofu.org/). Pulumi supports [170+ providers](/registry/), many derived from the same open-source Terraform provider schemas, plus native providers for AWS, Azure, and Kubernetes with no bridging layer.
...
CDK ships a unit testing library (`aws-cdk-lib/assertions`) that asserts against the synthesized CloudFormation template, in the same language as the app. Terraform testing lives outside the HCL language itself, in `terraform test` or third-party harnesses like Terratest, written in Go regardless of what deployed the infrastructure. Pulumi programs are ordinary code, so they use the same unit and property-testing frameworks (Jest, pytest, Go's testing package, and so on) already used for application code in that language, and those tests can run before any cloud call is made.

## How do the ecosystems and communities compare?

Terraform has the largest and oldest provider ecosystem by raw count (7,010 providers in the Terraform Registry), reflecting nearly a decade as the default IaC tool. OpenTofu, forked from Terraform in 2023 after HashiCorp's license change to BUSL, inherited that same provider compatibility and has grown its own module registry (23,600+ modules) under Linux Foundation governance. AWS CDK's ecosystem is the Construct Hub, a registry of reusable constructs scoped to AWS resources. Pulumi's registry spans 170+ providers, mixing native providers (built directly against cloud provider APIs, including Kubernetes and Azure Native) with providers bridged from Terraform's schemas, so most Terraform-supported services are also usable from Pulumi.
