...
That relationship is no longer one-directional. [Pulumi Cloud now manages Terraform and OpenTofu state](/docs/iac/get-started/terraform/terraform-state-backend/) as well, implementing the same remote backend API those CLIs speak, with [remote execution](/docs/iac/get-started/terraform/terraform-remote-execution/), approval gates on VCS-triggered applies, RBAC, and policy enforcement. So a team using Spacelift primarily as a managed state backend and runner for Terraform has a direct alternative, and a team using it to orchestrate a genuinely mixed estate still has a reason to keep it.
