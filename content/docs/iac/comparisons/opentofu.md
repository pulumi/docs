...
| Modularity and reuse | [Component Resources](/docs/iac/concepts/components/) authored in any supported language; [Pulumi Packages](/docs/iac/concepts/packages/) let a component written in one language be consumed from any Pulumi language; language-native package managers (npm, PyPI, NuGet, Maven, Go modules); and the [Pulumi Registry](/registry/) for publicly available packages | [Modules](https://opentofu.org/docs/language/modules/) referenced from local paths, Git, or registries; Pulumi can also [consume OpenTofu modules directly](/docs/integrations/terraform/modules/) and [host them in Pulumi Cloud's registry](/docs/integrations/terraform/module-registry/), where `tofu init` can still resolve them |
...
An existing investment in HCL, modules, and team expertise is no longer a reason on its own. Pulumi runs [HCL as a first-class language](/docs/iac/languages-sdks/hcl/) with OpenTofu-compatible provider resolution, [executes your existing OpenTofu modules](/docs/integrations/terraform/modules/), and can [back your OpenTofu state](/docs/integrations/terraform/state-backend/) without any change to your configurations. The two can also coexist — see [Adoption](#adoption-coexistence-conversion-and-import).
...
- [Using Pulumi Cloud as a Terraform or OpenTofu state backend](/docs/integrations/terraform/state-backend/)
