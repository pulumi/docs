...
| State management | [Managed by Pulumi Cloud by default](/docs/iac/concepts/state-and-backends/); DIY backends include Amazon S3, Azure Blob Storage, Google Cloud Storage, local files, and others | Stored in the Kubernetes cluster's etcd datastore as the status of resource objects; no separate state file |
...
| Execution model | Local CLI, programmatic via [Automation API](/docs/iac/concepts/automation-api/), or remote runs in [Pulumi Deployments](/docs/deployments/); the [Pulumi Kubernetes Operator](/docs/integrations/clouds/kubernetes/pulumi-kubernetes-operator/) enables GitOps-style continuous reconciliation from inside a cluster | Centralized: controllers run inside a Kubernetes cluster and reconcile resources continuously; changes are applied with `kubectl` or a GitOps workflow |
...
1. Want a GitOps operating model with continuous reconciliation — the [Pulumi Kubernetes Operator](/docs/integrations/clouds/kubernetes/pulumi-kubernetes-operator/) runs inside a Kubernetes cluster and reconciles Pulumi stacks continuously, with native support for Argo CD, Flux sources, and drift remediation, while programs remain authored in general-purpose languages.
