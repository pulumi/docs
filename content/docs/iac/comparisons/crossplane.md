...
| Execution model | Local CLI, programmatic via [Automation API](/docs/iac/concepts/automation-api/), or remote runs in [Pulumi Deployments](/docs/deployments/) | Centralized: controllers run inside a Kubernetes cluster and reconcile resources continuously; changes are applied with `kubectl` or a GitOps workflow |
