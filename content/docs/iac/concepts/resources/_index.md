...
- `CustomResource`: A custom resource is a cloud resource managed by a [resource provider](/docs/iac/concepts/providers/) such as AWS, Microsoft Azure, Google Cloud or Kubernetes.
- `ComponentResource`: A [component resource](/docs/iac/concepts/components/) is a logical grouping of other resources that creates a larger, higher-level abstraction that encapsulates its implementation details.
...
All resources have a required [`name`](/docs/iac/concepts/resources/names/) argument, which must be unique across resources of the same kind in a [`stack`](/docs/iac/concepts/stacks/). This *logical name* influences the *physical name* assigned by your infrastructure’s cloud provider. Pulumi [auto-names](/docs/iac/concepts/resources/names/#autonaming) physical resources by default, so the physical name and the logical name may differ. This auto-naming behavior can be overridden, if required.

The `args` argument is an object with a set of named property input values that are used to initialize the resource. These can be normal raw values—such as strings, integers, lists, and maps—or [outputs](/docs/iac/concepts/inputs-outputs/) from other resources. Each resource has a number of named input properties that control the behavior of the resulting infrastructure. To determine what arguments a resource supports, refer to that resource’s API documentation in the [Registry](/registry/).

The [`options`](/docs/iac/concepts/resources/options/) argument is optional, but lets you control certain aspects of the resource. For example, you can show explicit dependencies, use a custom provider configuration, or import existing infrastructure.

## Related topics

- [Resource Names](/docs/iac/concepts/resources/names/) - Learn more about resource names and how to use them.
- [Resource Options](/docs/iac/concepts/resources/options/) - Learn how to use resource options to modify the way that resources are managed by Pulumi.
- [Components](/docs/iac/concepts/components/) - Learn what a component resource is, how to author a new component resource, how to create child resources, and more.
...
- [Getter Functions](/docs/iac/concepts/functions/get-functions/) - Learn how a Pulumi resource uses its `get` function to retrieve a reference to an existing instance of the resource.
- [Provider Functions](/docs/iac/concepts/functions/provider-functions/) - Learn how to use the functions included with Pulumi packages.
