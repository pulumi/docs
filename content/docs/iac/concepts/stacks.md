...
1. **Back up both stacks' state** with `pulumi stack export --file backup.json` (repeat for the source and destination stacks, or Pulumi Cloud, which lets you download the state for any past update from the Stack Details page). A move is reversible with `pulumi stack import`, but only if you have a backup to restore from.
...
See the [`pulumi state move` CLI reference](/docs/iac/cli/commands/pulumi_state_move/) for the complete flag list, the [Move resources between stacks tutorial](/tutorials/move-resources-between-stacks/) for a guided walkthrough, and the [announcement blog post](/blog/move-resources-between-stacks/) for the background on why the command exists.
