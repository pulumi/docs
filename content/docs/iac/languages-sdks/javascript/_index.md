...
Pulumi templates typically include a specific TypeScript version in their `package.json`, which takes precedence over Pulumi's bundled compiler. See [TypeScript versions](#typescript-versions) below for supported version ranges, including guidance on [using TypeScript 7](#using-typescript-7).
