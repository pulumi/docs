...
title: "pulumi api | CLI commands"
aliases:
  - /docs/reference/cli/pulumi_api/
meta_desc: "Learn about the pulumi api command."
---
...
```
pulumi api [path-or-operation-id] [flags]
```

## Examples

```
  # Verify an op's parameters and schemas with `describe` before calling it.
...
```
      --all                     Follow pagination cursors and emit the combined result
  ...
  -X, --method string           HTTP method (default GET, POST when body fields are present)
      --output json             Drive content negotiation and rendering. Default uses the op's primary response content type (usually JSON). json or `markdown` request that format via the Accept header — rejected if the op's spec doesn't declare it. `raw` keeps the op's default Accept and writes the body through unchanged.
...
* [pulumi api describe](/docs/iac/cli/commands/pulumi_api_describe/)	 - Show the parameters and schemas for a Pulumi Cloud API operation
* [pulumi api list](/docs/iac/cli/commands/pulumi_api_list/)	 - List every Pulumi Cloud API endpoint
