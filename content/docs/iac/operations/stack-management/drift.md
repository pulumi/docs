...
- **[Pulumi Policy](/docs/insights/policy/)** enforces guardrails at preview time and, against the live cloud, in [Pulumi Insights](/docs/insights/) mode. Policy matters most when paired with drift handling: it prevents adoption from quietly normalizing a non-compliant change. If a teammate opens a 0.0.0.0/0 ingress in the console and the team decides to adopt the change, a policy that requires private CIDR ranges blocks the PR rather than letting the bad configuration enter the program. Policy in Insights mode also catches drift-introduced compliance violations in production even when no one runs a refresh.
