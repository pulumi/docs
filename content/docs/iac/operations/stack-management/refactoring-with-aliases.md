...
<<<<<<< HEAD
**Moving resources between stacks.** Use [`pulumi state move`](/docs/iac/cli/commands/pulumi_state_move/) to physically transfer resources from a source stack to a destination stack — see [Move resources between stacks](/docs/iac/concepts/stacks/#move-resources-between-stacks) for the full workflow. The command rewrites the moved resources' URNs to use the destination's stack and project, so the move itself doesn't require an alias. Add a `stack` and/or `project` alias on those resources in the destination's program when the destination's code declares them with a different identity than the source did, or when you migrated state through some other path (export/import, manual edits) and the URNs weren't rewritten for you.
=======
**Moving resources between stacks.** Use [`pulumi state move`](/docs/iac/cli/commands/pulumi_state_move/) to physically transfer resources from a source stack to a destination stack. The command rewrites the moved resources' URNs to use the destination's stack and project, so the move itself doesn't require an alias. Add a `stack` and/or `project` alias on those resources in the destination's program when the destination's code declares them with a different identity than the source did, or when you migrated state through some other path (export/import, manual edits) and the URNs weren't rewritten for you. See [Moving resources between stacks](/docs/iac/operations/stack-management/moving-resources-between-stacks/) for the full workflow, including how to update program code on both sides.
>>>>>>> origin/master
