...
{{% notes type="info" %}}
If your Pulumi issue is causing a serious outage for your workload(s), and Pulumi CLI operations are taking too long, consider the [`--target` option](/docs/iac/operations/stack-management/targeted-updates/), which allows you to limit the operation to only the resources you specify. The flag is supported on `pulumi refresh`, `pulumi preview`, `pulumi up`, and `pulumi destroy`, and can be specified more than once.
{{% /notes %}}
