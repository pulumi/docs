...
Pulumi Cloud includes a managed [deployment service](/docs/deployments/concepts/) that runs Pulumi operations remotely rather than from a local machine—for example, in response to Git pushes or on demand. It can also send [webhooks](/docs/deployments/concepts/webhooks/) for events such as deployments, resource updates, and policy violations, which you can use to trigger custom, event-driven workflows.
