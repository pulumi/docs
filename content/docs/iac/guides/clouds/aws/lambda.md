...
aliases:
- /docs/integrations/clouds/aws/guides/lambda/
...
{{% notes type="warning" %}}
Magic Lambda functions are not supported when using the Bun runtime (`runtime: bun`), as they rely on [function serialization](/docs/iac/concepts/functions/function-serialization/), which requires Node.js v8/inspector APIs that Bun does [not fully implement yet](https://bun.com/docs/runtime/nodejs-compat#nodeinspector). Use `runtime: nodejs` if your program uses magic Lambda functions.
{{% /notes %}}
