...
- [Azure Pipelines task extension](https://marketplace.visualstudio.com/items?itemName=pulumi.build-and-release-task) — see the [Azure DevOps guide](/docs/iac/guides/continuous-delivery/azure-devops/).
- GitHub Actions — [`pulumi/actions`](https://github.com/pulumi/actions) and [`pulumi/action-install-pulumi-cli`](https://github.com/pulumi/action-install-pulumi-cli); see the [GitHub Actions guide](/docs/iac/guides/continuous-delivery/github-actions/). GitHub-hosted runners ship the Pulumi CLI pre-installed, so an explicit install step is only needed to pin a specific version.
- [CircleCI orb](https://circleci.com/developer/orbs/orb/compute/pulumi) — see the [CircleCI guide](/docs/iac/guides/continuous-delivery/circleci/).
