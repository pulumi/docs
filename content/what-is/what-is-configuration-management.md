...
about:
  ...
  sameAs: https://en.wikipedia.org/wiki/Configuration_management
keywords:
...
- **Configuration management keeps systems** — servers, networks, applications — **in a defined, repeatable, and auditable state.**
- **The three jobs of a configuration management tool**: identification (what's drifted), remediation (fix it), and automation (do it without humans).
- **Common tools** include Ansible, Chef, Puppet, and SaltStack — they were built for the "long-lived VM" era and target patching, package installation, and OS config.
- **Configuration management overlaps with but is distinct from [infrastructure as code](/what-is/what-is-infrastructure-as-code/)**: IaC provisions the resource; configuration management tunes its inner state.
- **In modern cloud architectures**, much of what configuration management used to do has moved into immutable images, container builds, and Kubernetes manifests.
...
## Get started managing configuration with Pulumi

1. **Provision the infrastructure** (VMs, clusters, networks) with `pulumi new` and `pulumi up`.
1. **Drive in-VM config** with a configuration management tool like Ansible, called from your Pulumi program — see this [Pulumi + Ansible walkthrough](/blog/deploy-wordpress-aws-pulumi-ansible/).
1. **Manage cross-environment configuration** with [Pulumi ESC](/product/esc/) so config values, secrets, and settings stay in sync across dev, staging, and prod.
