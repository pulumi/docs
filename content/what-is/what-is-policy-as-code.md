...
Pulumi Policies is Pulumi's policy as code engine, built into the [Pulumi platform](/docs/discovery-governance/concepts/policy-as-code/). It runs during every `pulumi preview` and `pulumi up`, evaluating resources against a set of policies before any change reaches the cloud.
...
The combination of [policy as code enforcement](/docs/discovery-governance/concepts/policy-as-code/) with [Discovery-based governance](/product/discovery-governance/) let Modivcare start enforcing cost controls on resources they didn't even write with Pulumi, reducing infrastructure costs by up to 25%.
