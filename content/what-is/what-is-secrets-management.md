...
about:
  name: Key management
  sameAs: https://en.wikipedia.org/wiki/Key_management
keywords:
  - secrets management
  - key management
  - secrets storage
...
- **Secrets management is the secure storage, distribution, rotation, and revocation** of sensitive credentials — API keys, passwords, certificates, and tokens.
- **The goal is to keep secrets out of source control**, environment variables, and developer laptops, and to make rotation and revocation a routine operation.
- **Common backends** include [AWS Secrets Manager](/what-is/what-is-aws-secrets-manager/), [Azure Key Vault](/what-is/what-is-azure-key-vault/), [Google Cloud Secret Manager](/what-is/what-is-google-cloud-secret-manager/), and [HashiCorp Vault](/what-is/what-is-hashicorp-vault/).
- **Best practice is to use dynamic, short-lived credentials** wherever possible, not long-lived static secrets — they limit blast radius if exposed.
- **Secrets management is foundational for compliance** ([SOC 2](/what-is/what-is-soc-2/), [HIPAA](/what-is/what-is-hipaa/), PCI DSS) — auditors expect rotation, access logging, and least-privilege controls.
...
## Get started managing secrets with Pulumi

1. **Set encrypted config and secrets** on a stack:

   ```bash
   pulumi config set --secret dbPassword 'hunter2'
   ```

1. **Pull dynamic secrets at deploy time** from [AWS Secrets Manager](/what-is/what-is-aws-secrets-manager/), [Azure Key Vault](/what-is/what-is-azure-key-vault/), [Google Cloud Secret Manager](/what-is/what-is-google-cloud-secret-manager/), or [HashiCorp Vault](/what-is/what-is-hashicorp-vault/) using [Pulumi ESC](/product/esc/).
1. **Rotate and audit** centrally through Pulumi ESC environments rather than tracking secrets across many providers.
