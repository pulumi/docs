...
meta_desc: "Run 'aws ec2 describe-instances' with short-lived, OIDC-scoped AWS credentials brokered by Pulumi ESC. No static keys, fully auditable in CloudTrail."
...
- **Scoped to one IAM role.** The role's policy is the upper bound for what the session can do. A read-only EC2 inventory role can list instances and nothing else.
...
- An IAM role with `ec2:DescribeInstances`, and a trust policy that allows Pulumi's OIDC issuer to assume it. Use the [Pulumi + AWS OIDC guide](/docs/esc/guides/configuring-oidc/aws/) to set it up.
- The AWS CLI v2 installed.

## Step-by-step: set up dynamic credentials for EC2
...
```bash
$ esc env init <your-pulumi-org>/<your-project>/aws-ec2-read
```
...
## Run `aws ec2 describe-instances` with dynamic credentials

```bash
$ esc run <your-pulumi-org>/<your-project>/aws-ec2-read -- \
...
- `--region` picks the AWS region — EC2 is region-scoped, and `describe-instances` only returns instances in the region you query.

For a tighter view, add filters and an output projection:
...
```bash
$ esc run <your-pulumi-org>/<your-project>/aws-ec2-read -- aws sts get-caller-identity
{
...
| `UnauthorizedOperation: ... ec2:DescribeInstances` | Role's identity policy lacks the action | Add `ec2:DescribeInstances` to the role |
...
### Can I use `esc run` in CI for nightly inventory?

Yes — this is a canonical use case. Set `PULUMI_ACCESS_TOKEN` on the runner and invoke `esc run <env> -- aws ec2 describe-instances`. The runner needs no long-lived AWS credentials. Pulumi's GitHub Actions, GitLab CI, and CircleCI integrations all support this pattern.

### What is the minimum IAM permission the role needs?

`ec2:DescribeInstances` and nothing else. The action does not support resource-level constraints — the policy resource must be `"*"`. Constrain by `aws:RequestedRegion` if you need to bound it to one region. Tag-based scoping is available via the `ec2:ResourceTag/<key>` condition key for write actions, but not for `DescribeInstances`.
...
### Can I scan multiple regions in one script?

Yes. Loop through your regions and pass `--region` to each `aws ec2 describe-instances` call inside a single `esc run` invocation, or run one `esc run` per region. The role is assumed once per `esc run`; each AWS call inside reuses the same session.
