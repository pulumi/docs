...
| Policy as code | [Pulumi policy as code](/docs/discovery-governance/concepts/policy-as-code/), Kyverno, OPA Gatekeeper |
...
* **Enforce policy in two places.** In CI, [policy as code](/docs/discovery-governance/concepts/policy-as-code/) blocks non-compliant changes from merging at all. In the cluster, admission controllers (Kyverno, OPA Gatekeeper) backstop anything that arrives by another path. The CI check is faster feedback; the admission controller is the last line of defense.
...
* **Codify policy.** No naked pods, no privileged containers, no `:latest` tags in production, mandatory resource requests and limits, mandatory liveness/readiness probes. Enforce in CI with [Pulumi policy as code](/docs/discovery-governance/concepts/policy-as-code/) or in the cluster with Kyverno / OPA Gatekeeper.
