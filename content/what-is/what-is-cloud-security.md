...
Cloud security posture management (CSPM), infrastructure as code, [policy as code](/docs/discovery-governance/concepts/policy-as-code/), drift detection, and automated remediation.
...
* **Enforce policy as code in CI.** Block insecure configurations before they deploy with tools like [Pulumi Policies](/docs/discovery-governance/concepts/policy-as-code/) or Open Policy Agent.
...
* **Policy as code blocks insecure changes.** [Pulumi Policies](/docs/discovery-governance/concepts/policy-as-code/) run in CI alongside `pulumi preview`, so a public bucket or a `0.0.0.0/0` ingress rule never reaches production.
...
1. Run a CSPM or [Pulumi Policies](/docs/discovery-governance/concepts/policy-as-code/) pass to find public storage, wildcard IAM, and open ports.
