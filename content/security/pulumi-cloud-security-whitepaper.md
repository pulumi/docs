...
meta_desc: Technical overview of Pulumi Cloud security architecture, cryptographic controls, key management, and operational security practices.
layout: security
---
...
The Pulumi Cloud platform is an infrastructure-as-code delivery system that enables organizations to define, deploy,
and manage cloud infrastructure through declarative programming interfaces. This
...
flowchart TD
    ...
    %% Pulumi Cloud Core
    subgraph PulumiCloud["Pulumi Cloud Service"]
        direction TB
        CoreAPI[Core Service Layer: RESTful APIs for all platform operations]

        subgraph Components["Specialized Components"]
...
resources are organized into horizontally scalable clusters provisioned with capacity headroom, while
execution-intensive workloads run on an elastic worker pool that provisions isolated capacity on demand. Services are
distributed across multiple availability zones to ensure high availability, with load balancing distributing incoming
traffic across healthy service instances. The managed service is operated in multiple geographic regions, allowing
organizations to choose where their data resides. The deployment model supports both fully managed cloud deployments
and self-hosted installation options for organizations with specific regulatory or operational requirements.
...
provides request routing, rate limiting, and protection against common denial-of-service attacks. Application services
reside in private network segments with no inbound internet exposure, communicating through internal service discovery
mechanisms. Database and storage resources are further isolated in dedicated network zones. Restrictive access controls
allow only authorized application connections.
...
representing organizations, projects, stacks, deployment history, and configuration state. It implements role-based access
control, including organization-defined custom roles and attribute-based conditions, ensuring that all operations
respect organizational access policies and user permissions.

User authentication supports multiple mechanisms, including federation with organizational identity providers for
...
scanning component connects to cloud provider APIs to inventory existing infrastructure resources, building a catalog
of deployed assets. The evaluation component assesses these resources against organization-defined
...
The system implements human-in-the-loop governance: modifying operations require explicit approval of the proposed
plan before execution, whether granted in the conversation itself, through pull request review, or through dedicated
change-request workflows. Agent workloads execute in isolated, single-use environments and act through short-lived,
narrowly scoped credentials rather than standing user authority. This architectural pattern ensures that automated
infrastructure modifications remain subject to organizational oversight and approval processes, maintaining control
over production environments while leveraging AI capabilities for operational efficiency.
...
Database resources accept connections only from authorized application services across isolated internal network
segments. The platform encrypts data at rest using provider-managed encryption services.
...
storage systems. The platform stores rate-limiting state, metadata caches, and query results in managed cache clusters
deployed across multiple availability zones with automatic failure detection and failover. The caching layer implements
appropriate cache invalidation strategies to maintain consistency while maximizing hit rates.
...
Request compression reduces bandwidth consumption on high-volume operations, while retry logic handles transient
network failures. When tracing is enabled, distributed tracing headers are injected into requests, enabling end-to-end
...
flowchart TD
    ...
    subgraph Backend
        ...
        D2[DIY Backends #40;Object Storage, Filesystem, Database#41;]
    end
...
The platform implements a three-tier key hierarchy that separates key management responsibilities and enables flexible
key rotation without requiring re-encryption of all data. At the top of the hierarchy are key encryption keys, which
are never stored within the platform's data stores. For cloud deployments, these keys reside in external key management
services operated by cloud infrastructure providers. Self-hosted deployments can likewise integrate an external key
management service, or use local key material protected by operating system access controls.

Data encryption keys form the second tier of the hierarchy. These symmetric keys are generated using cryptographically
secure mechanisms and are used for the actual encryption of content. Data encryption keys are
...
Envelope structures encode the encryption format version, initialization vectors, authentication tags, and cryptographic
binding tags alongside the ciphertext. Format versioning enables cryptographic agility, allowing
the system to migrate to new encryption algorithms or parameters as security requirements evolve. Binding tags tie
encrypted content to specific encryption keys, preventing key substitution attacks where an attacker might attempt to
decrypt content using a key they control.
...
The platform integrates with key management services operated by major cloud infrastructure vendors. Organizations can
optionally supply customer-managed keys, maintaining control over their own key material while leveraging
provider-operated security infrastructure. These integrations support standard key operations including data key
generation, key encryption, and key decryption.

When generating new encryption keys, the platform obtains both a plaintext and an encrypted version of the key. The
plaintext key is used only transiently for the encryption operation and is never persisted; only the encrypted key is
stored in the database, associated with the encrypted content through cryptographic binding mechanisms.
...
keys in their own key management service accounts. The platform integrates with these customer-managed keys through
...
potential key compromise. The platform implements key migration capabilities that allow systematic re-encryption of
data from one key encryption key to another without service interruption.
...
keys throughout the operation. Migration state is tracked persistently, allowing failed migrations to be safely retried
...
All network communication with external clients, and all service traffic that crosses public networks, utilizes
transport layer security protocols with enforced minimum protocol versions. Modern cipher suites providing forward
secrecy are preferred, ensuring that compromise of long-term keys does not enable decryption of historical traffic.
Certificate validation is enforced for these connections, with certificates issued by trusted certificate authorities.

The command-line interface enforces transport layer security by default for all communication with cloud services. An
insecure mode exists to facilitate development and testing scenarios, but it is never enabled by default and requires
an explicit opt-in at login time.
...
When users define secrets in their infrastructure programs, secret values are encrypted before they are stored, and
only ciphertext is ever written to state checkpoints. With the default managed secrets provider, encryption is
performed by the backend service using the envelope encryption mechanism described previously, with values protected in
transit by transport layer security. Organizations that require secret material never to reach the service in plaintext
can instead select a client-side secrets provider, in which encryption occurs entirely within the command-line
interface and the backend service only ever handles ciphertext. In either model, decryption occurs only when an
infrastructure operation requires access to the secret value, such as during resource provisioning.

The command-line interface provides multiple secrets manager implementations to accommodate different deployment models
and security requirements. Users can select backend service encryption, passphrase-based encryption, cloud-native key
management services, or a dedicated external secrets management system. Regardless of implementation choice, the user
interface remains consistent, allowing users to switch encryption backends without modifying their infrastructure code.

### Diagnostic Log Protection

Diagnostic logs produced by the command-line interface can capture fragments of sensitive information, so the platform
extends its encryption architecture to client-side log content. Clients encrypt the logs they produce locally using
symmetric session keys generated freshly for each session. These session keys are protected through the envelope
encryption hierarchy described previously: only the wrapped form of a key is persisted by the platform, while the
plaintext key is released once to the requesting client and never stored. Encrypted logs remain on the user's machine
until the user actively chooses to share them, for example to assist a support investigation. Shared logs can be read
only through a restricted administrative key-release path, which is subject to the platform's access-enforcement and
audit controls.
...
Audit logging captures significant operations performed within the platform, providing accountability and enabling
security monitoring, compliance reporting, and incident investigation. Audit events include user authentication,
authorization decisions, resource modifications, administrative actions, and configuration changes. Each audit event
records the acting user, timestamp, source network address, and a description of the action taken and the resources
affected.

Administrative operations receive special audit treatment due to their elevated privileges. A dedicated audit log
captures administrator commands with detailed context including business justification references and the affected
accounts. This separated administrative audit trail ensures that privileged operations receive appropriate scrutiny;
records are written through insert-only paths, and no administrative interface permits modifying them after the fact.

Organizations can configure automated export of their audit logs to object storage under their control. Events are
delivered on a regular schedule in a standard security-event format, enabling organizations to integrate platform audit
activity with their existing security information and event management systems.

### Monitoring and Observability

Monitoring infrastructure provides real-time visibility into platform operations, enabling rapid detection and response
to performance degradations, service disruptions, and security incidents. The monitoring architecture follows the three
pillars of observability: metrics, logs, and distributed traces.
...
performance overhead and cost of trace collection, with intelligent sampling that captures server errors and slow
requests in full while sampling only a fraction of successful fast requests.
...
providing detailed records for post-incident analysis.
...
latest security patches are deployed even when application code has not changed. Images are scanned for known
vulnerabilities in system packages and application dependencies as they are built and published, and the compute hosts
that run them undergo continuous vulnerability scanning. Critical vulnerabilities trigger accelerated update processes
that expedite patching outside normal release cycles.
...
Data durability and availability are ensured through layered backup strategies that protect against hardware
...
disasters do not impact backup availability, and are retained according to defined retention policies. Snapshot
restores are exercised in non-production environments, preventing scenarios where backup processes appear successful
but generate unrestorable backups.
...
instances fail, remaining healthy instances continue serving traffic at potentially reduced capacity, and standing
headroom allows additional instances to be brought online quickly to restore service levels. This reserve capacity
ensures that temporary increases in load or partial infrastructure failures do not result in complete service
unavailability.
...
maintaining strong security guarantees. Audit logging and monitoring enable both real-time security event detection
and post-facto incident investigation.
