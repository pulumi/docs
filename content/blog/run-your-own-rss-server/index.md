...
We'll start by creating a new Pulumi project. (If you're completely new to Pulumi, it might be good to begin with our [Getting Started guide](https://www.pulumi.com/docs/get-started/), which walks you through installing Pulumi and configuring it for your cloud provider.) We'll use the built-in `aws-typescript` project template:
...
Once you've stepped through the prompts, you should be left with a minimal `index.ts` file that defines and exports an S3 bucket &mdash; but since we won't be needing an S3 bucket for this project, we can remove that code, and keep only the imports we'll need:
...
You can change any of these values if you like (and you should definitely change the two passwords, of course), but note that the passwords are set with the `--secret` flag, which encrypts their values using Pulumi's [built-in support for secrets](https://www.pulumi.com/docs/intro/concepts/config/).
...
* export the URL as a Pulumi stack [Output](https://www.pulumi.com/docs/intro/concepts/programming-model/#outputs) so we can navigate to the service.
...
Technically, we're done &mdash; but there are a couple of things we might like to do to make this project a bit more complete.
...
Run `pulumi up` one last time, and you should see your service now running on port 443 &mdash; but you won't be able to access it until you add CNAME record to the DNS settings for your domain (I happen to use Google for mine) mapping a new subdomain to your load balancer's hostname:
