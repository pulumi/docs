...
- You can use [component resources](/docs/concepts/resources/#components) to group your resources into logical groups - similar to how Terraform modules work. If you don't do this there's a high risk you'll end up with a very messy setup further down the line.
- Read Pulumi's own [overview](/docs/concepts/) and [naming](/docs/concepts/resources/#autonaming) docs before attempting to use Pulumi yourself. I'd say that's the most important bit to know about.
...
I started by testing out the [AWS getting started guide](/docs/iac/get-started/aws/) to get a feel for how Pulumi worked. This was a great move and I recommend everyone else to do the same.
...
You can check the [API reference](/registry/packages/aws/api-docs/s3/bucket/) for details on how to import each specific resource. The instructions on how to do it is usually at the bottom of the reference page for each resource type.
...
When your infrastructure starts growing it becomes hard to manage unless you group your resources. Pulumi's solution to this are the so called [component resources](/docs/concepts/resources/#components). They are "logical components" which you define yourself.
...
Each resource in pulumi has a globally unique [URN](/docs/concepts/resources/#urns). You can show the URNs for the infrastructure you've codeified in pulumi by issuing `pulumi stack --show-urns`
...
This might seem tricky at first, but digest this for a bit, then have a look at the docs for [Inputs and Outputs](/docs/concepts/inputs-outputs/) and I'm sure it'll all make sense.

### Don't name resources unless you really have to

Pulumi is pretty smart when it comes to naming the actual resources on AWS. If you don't specify a `name` when creating a resource Pulumi will use the identifier and [append a hex character suffix](/docs/support/faq/#why-do-resource-names-have-random-hex-character-suffixes) .
...
You'll have to make use of your gitlab-fu (or other-ci-cd-fu) to adapt this to your own needs. Read the [docs from Pulumi](/docs/iac/packages-and-automation/continuous-delivery/) to get started. It's usually very simple and straight forward!
...
You are normally able to have Pulumi post a report of the planned changes [attached to your merge request](/docs/iac/packages-and-automation/continuous-delivery/gitlab-app/) .
