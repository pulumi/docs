...
This blog post recreates the existing [Typescript voting app example](/dev/examples/aws-ts-voting-app/) step by step in Python with Flask as the frontend and Redis as the backend. In future blog posts, we will explore how to change the front and backends, how to upgrade the app with additional AWS services, and migrating from one cloud provider to another.

The first step is to clone the [Typescript voting app example](/dev/examples/aws-ts-voting-app/). We'll use a sparse clone to copy only the `aws-ts-voting-app` directory.
