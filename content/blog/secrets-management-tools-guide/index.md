...
Dynamic credential generation sets Pulumi ESC apart from traditional storage-focused tools. Instead of storing long-lived AWS access keys or Azure service principal secrets, it automatically provisions short-lived OIDC tokens on-demand. This [dynamic approach](/docs/esc/providers/) eliminates entire categories of security risk by ensuring credentials expire automatically and can't be misused over extended periods, sidestepping the [AWS CLI credential errors that long-lived keys eventually cause](/what-is/aws-cli-dynamic-credentials-with-pulumi-esc/) in the first place.
