...
We're going to start by adding settings, both secret and plaintext, to our [stack configuration](/docs/iac/concepts/config/), in a [structured way](/docs/iac/concepts/config/#structured-configuration). We're going to add these settings as a dictionary so we can use the names as the variable names in our Lambda Function.
...
Before we continue, it's worth pointing out the following (from the [Pulumi documentation on secrets](/docs/iac/concepts/secrets/#a-warning-using-secrets-in-code) ):
