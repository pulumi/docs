...
last leg of the continuous delivery process to update your AWS resources and how to securely store sensitive data using Pulumi.
<!--more-->
...
limited privledges. Then in [part 2]({{< relref "/blog/managing-aws-credentials-on-cicd-part-2" >}}) we set up a simple,
serverless Pulumi program that periodically rotated the User's access keys and updated the CI/CD system.

In this final post, we will now _use_ those AWS credentials so that we can update cloud resources as part of
...
think that we should just use that same AWS access key in order to gain access to AWS resources. However, using the
access keys for an IAM User has a few major drawbacks.

First, those access keys are long-lived. If we didn't create that Pulumi program to automatically rotate them, the
key would last forever. (Meaning that a bad actor, perhapse elsewhere, or far in the future, could take advantage
...
Instead, an IAM Role defines a set of capabilities that can be performed by any principle capable of assuming it. So
in other words, you can exchange your limited access IAM User's credentials for a specialized IAM Role. The IAM Role then
can be specifically tailored to a specific... wait for it... _role_ to perform.

Don't worry if that doesn't all make sense at first, let's look at a concrete example.

### Desired End State

In our situation, we want to create an IAM Role `WebsiteStackUpdaterRole` that grants that set of permissions necessary for
...
> This is a situation where Pulumi being able to declare the policy document in code is super-useful, as it makes it
...
First, the "principle" (who can assume the role) references a specific AWS account. This is an example of how you
can associate one AWS account with another. (And is best practice for isolating data between different departments, etc.)
We'll cover what configuration we'll need to do on the AWS bastion account side to use this role later.

The next thing to call out is the use of the `Condition` element of the policy document. In order to assume the role
we require two checks be performed.

- That the principle assuming the role (e.g. IAM User) have the tag `"access-to-production"` with value `"true"`. This way,
...
- We also verify that the "external ID" (an optional parameter passed when assuming an IAM Role) matches a well-known Pulumi
  stack name. This way we can double check that the entity who is using the IAM Role is doing so to use a specific stack.
  (This is less of a security measure and more of a general hygine type requirement.)

### IAM Policy Document

Now we have an IAM Role, we need to attach a policy document that defines what permissions are actually granted.

This is the **most important step**, since it will define what exactly the CI/CD bot will be able to do.
...
> It can be extremely useful to have an identical "staging" and "production" instance of the same Pulumi stack,
...
> Reminder, we defined the IAM Role in our production AWS account which allows any principle in the AWS
...
Finally! With everyone in place, we can now can _use_ the IAM Role.

The following is a set of commands you can run on your CI/CD worker to exchange the low-priledge IAM User
credentials, for the specific IAM Role we just created in the production AWS account. But there are many
alternatives for how to programatically assume an IAM Role, such as using the
...
# These are the low-priledge credentials that automatically get rotated.
...
ultimately where you (or your customer's) data is stored. But beyond that, there are places where
sensitive data gets stored by Pulumi too.

Pulumi goes to great lengths to safeguard your data. But here we review where Pulumi interacts
with your sensitive data, and what you can do to better safeguard it.

### Secrets in Configuration

A Pulumi stack can have configuration data associated with it, which allow you to parameterize
...
along with the source code for a stack, so that builds and stack updates are reproduceable.

However, what if those configuration settings contains secrets? Like you need to store an API key
...
data is encrypted using a key specific to your stack. (So the cipher text stored in the `Pulumi.yaml` file
...
Securely managing any sensitive data is difficult to get right. Especially if there are a number of moving parts, such as when
...
IAM Users and Roles. And then reviewed the specifics of where and how Pulumi stores sensitive data, in case you want to take
