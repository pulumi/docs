...
- An [azure-login](/docs/esc/providers/login/azure-login/) environment
...
Create a new environment for your rotator. If you have the existing credentials, set them in the [state](/docs/esc/concepts/builtin-functions/fn-rotate/#parameters) object so the rotator will treat them as the `current` credentials.
...
The `lifetimeInDays` field controls how long each generated secret remains valid before it expires. Azure allows a maximum of 730 days (two years), but shorter lifetimes are recommended for better security. Make sure to set a rotation [schedule](/docs/esc/concepts/rotators/#schedule) that runs before the lifetime expires so your credentials are always fresh.
...
The `fn::rotate::azure-app-secret` rotator is available now in all Pulumi ESC environments. For more information, check out the [fn::rotate::azure-app-secret documentation](/docs/esc/providers/rotators/azure-app-secret/)!
