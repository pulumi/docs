...
To accomplish the task of provisioning all the required resources, the team wrote a [Pulumi Component](/docs/iac/concepts/components/) to encapsulate all the complexity and provide a simple interface for provisioning and updating the service. Unfortunately, this was before we had developed [Pulumi Packages](/docs/iac/guides/building-extending/providers/debugging-providers/) and the Component was only available within the program itself.

## The Metabase Package

That brings us to today, and we are happy to deliver an open source Pulumi Package for running Metabase on [AWS ECS](/docs/iac/guides/clouds/aws/ecs/). With Pulumi’s Metabase Package, you can quickly get started with Metabase without having to worry about the underlying infrastructure.
