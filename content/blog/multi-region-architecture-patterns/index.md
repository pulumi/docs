...
Picking a [pattern](#which-multi-region-pattern-fits-active-active-warm-standby-pilot-light-or-backup-and-restore) only protects the layer you deliberately replicated. Most multi-region designs still have single points of failure hiding in the layers nobody put in the diagram:

- **DNS and edge routing.** If a single DNS provider or CDN sits in front of both regions, a fault there takes down access to both regions at once, regardless of how well the backend failed over. Cloudflare's November 2025 outage affected traffic across its entire network, regardless of where it was headed — a bad configuration file reached every machine, not a subset tied to one region.
...
- DynamoDB: [`aws.dynamodb.Table`](https://www.pulumi.com/registry/packages/aws/api-docs/dynamodb/table/) with a `replicas` list naming every region — this is Global Tables V2, the current form. The older `aws.dynamodb.GlobalTable` resource manages Global Tables V1, layered on top of DynamoDB tables that already exist in each region.
...
1. **Review stacks on every pull request.** [Review stacks](https://www.pulumi.com/docs/deployments/concepts/review-stacks/) — a Pulumi Cloud feature, not available with self-managed state backends — preview the multi-region diff on every PR touching this code, so a regression in one region's configuration shows up in code review, not in an incident.
