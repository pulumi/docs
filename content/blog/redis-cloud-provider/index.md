...
[Redis Enterprise Cloud](https://redis.com/redis-enterprise-cloud/overview/) is a fully managed database as-a-service (DBaaS) provider that offers managed Redis clusters. Redis Enterprise Cloud clusters can be accessed via network peering in AWS or Google Cloud, or over the internet. The [Redis Cloud provider](/registry/packages/rediscloud/) allows you to manage Redis Enterprise Cloud resources in Pulumi programs.
...
Then, in your `index.ts` file, add use the [`getPaymentMethodOutput`](/registry/packages/rediscloud/api-docs/getpaymentmethod/) function to obtain your payment information, and use the resulting card ID to create a Redis Subscription:
...
Now you are ready to add your AWS resources. First, you'll need to add some reference to your Pulumi program. Add the Pulumi [AWS Provider](/registry/packages/aws/), which provides basic AWS resources, and the [AWSX package](/registry/packages/awsx/), which provides a convenient [VPC component](/registry/packages/awsx/api-docs/ec2/vpc/):
...
{{% notes type="info" %}}
If you are unfamiliar with the call to `apply()`, you can learn more about why this is necessary in [Inputs and Outputs](/docs/concepts/inputs-outputs/#apply). Creating resources within `apply` is usually not necessary, but this is one of the few cases where it is.
{{% /notes %}}
...
* Replace the EC2 test instance with a real workload that uses Redis:
  ...
  * If you're using another AWS compute platform, Pulumi also has helpful guides for creating workloads on [ECS](/docs/iac/clouds/aws/guides/ecs/), [EKS](/docs/iac/clouds/aws/guides/eks/), or [Lambda](/docs/iac/clouds/aws/guides/lambda/).
* Try creating [Redis Cloud clusters peered with Google Cloud](/registry/packages/rediscloud/api-docs/subscriptionpeering/#gcp).
* If you're comfortable using Pulumi and are thinking of adopting it at a team or organizational level, consider [creating an organization in Pulumi Cloud](/docs/pulumi-cloud/admin/organizations/) and enjoying a [2-week free trial of our full set of features](https://app.pulumi.com/signup), no credit card required!
