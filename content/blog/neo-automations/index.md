...
Recurring platform work slips: provider versions fall behind, drift accumulates between checks, and the quarterly audit keeps getting pushed back another month. [Pulumi Neo](/blog/pulumi-neo/) can now run any [task](/docs/ai/neo/tasks/) on a cadence you set, opening a pull request for each run.
...
Automations default to two settings that fit recurring work. Approval mode is [**auto**](/docs/ai/neo/tasks/#task-modes), so a run doesn't wait for human confirmation between steps. Permission mode is [**read-only**](/docs/ai/neo/tasks/#task-modes), so a run can read state and propose changes through pull requests but can't apply changes directly. You can override either default per automation.
