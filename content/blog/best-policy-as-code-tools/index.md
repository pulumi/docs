...
social:
    ...
    linkedin: |
        "Policy as code" and "IaC security scanning" get lumped together constantly, and they solve different problems. A scanner tells you a resource is misconfigured. A policy engine decides, before the change is applied, whether it is allowed to happen at all.
    ...
    bluesky: |
        Policy as code vs. IaC security scanning: not the same category. We compared OPA, Sentinel, Kyverno, Checkov, and Pulumi Policies on what they actually enforce, and when.
---
...
A policy engine evaluates a proposed change against rules you write, and it can block that change from ever being applied. A security scanner inspects code or a running environment and reports what it finds; some can open a fix as a pull request, but none of them sit in the deployment path with the authority to stop it. The table below draws that line using Checkov as the bridge case, since it legitimately does both.
...
| AI-assisted code security tools (gomboc.ai, CodeAnt AI) | Vendor's rule set, applied by an AI reviewer | In CI or as an assistant reviewing pull requests | No; typically opens a remediation PR for a human to merge | Finding and proposing fixes for security issues in code |
| Cloud security platforms (SentinelOne and similar CNAPPs) | Vendor's rule set | Continuously, against running cloud accounts | No; alerts and prioritizes findings | Runtime visibility, posture management, and threat detection across an entire cloud estate |
