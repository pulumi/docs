...
With Pulumi, the entire path &mdash; ECR repository, Docker build and push, IAM role, Lambda function, and event source &mdash; is one program in TypeScript, Python, Go, .NET, Java, or YAML. The rest of this post walks through that program with a real video-thumbnailer example.
...
- **Duration**: $0.0000166667 per GB-second on x86_64; arm64 (Graviton2) is up to 34% cheaper.
...
[Pulumi Crosswalk for AWS](/docs/iac/clouds/aws/guides/) wraps the ECR repository, the Docker build, and the push into two resources:
...
<script type="application/ld+json">
{
  ...
  "step": [
    ...
    },
    {
      ...
      "text": "Declare the S3 bucket the function will read from and write to using aws.s3.BucketV2.",
    ...
    },
    {
      ...
      "text": "Use the bucket's onObjectCreated event helper or a BucketNotification plus lambda Permission to invoke the function on new .mp4 uploads.",
...
- Read the [Pulumi AWS Lambda guide](/docs/iac/clouds/aws/guides/lambda/) for event sources, IAM patterns, and packaging tips.
