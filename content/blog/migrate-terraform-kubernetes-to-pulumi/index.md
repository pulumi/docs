...
An AI coding agent can drive most of this conversion and import work for you. [Neo](/product/neo/) has the advantage of already knowing this flow, but a general-purpose agent like Claude Code, Cursor, or Codex can do the hand conversion too. This guide covers the same ground manually: useful when you want to review and adjust each step yourself, or when part of your config (`kubernetes_manifest`, CRDs) needs the kind of judgment calls that are worth understanding regardless of which path you take. The [Terraform migration guide](/docs/iac/guides/migration/migrating-to-pulumi/from-terraform/) covers the general, non-Kubernetes-specific version of this same migration, including `pulumi-terraform-migrate` as a third, state-first option.
...
If your Terraform state has more resources than you want to import one at a time, `pulumi import --from terraform ./terraform.tfstate` reads a `.tfstate` file directly and imports every managed resource in it in one pass, marking each protected in the process. It's the fastest route for a config made up mostly of the typed resources from the table above. Reach for the per-resource form instead when you're importing selectively (bringing one namespace across at a time during a staged cutover, for example) or adopting a `kubernetes_manifest` or CRD instance you've already hand-rewritten as a `CustomResource`, since `--from terraform` can only import what the converter can map, and `kubernetes_manifest` and CRD instances are precisely the rows it can't.
