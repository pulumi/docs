...
- When config tweaks start firing your full CI pipeline unnecessarily, it's time to split.
...
{{% notes type="info" %}}
Pro Tip: When you do split, a common pattern is two repos: one for application source code, one for deployment configuration (Helm charts, Kustomize overlays, environment-specific values). Some larger orgs go further with a third repo for environment overrides, following the "fleet repo, infra repo, apps repo" model from the FluxCD maintainers. Either way, pair it with image update automation (Flux Image Automation Controller or ArgoCD Image Updater) so image tag bumps across repos don't turn into manual toil.
{{% /notes %}}
...
One nuance worth calling out: the [rendered manifests pattern](https://akuity.io/blog/the-rendered-manifests-pattern) does use per-environment branches, and you'll see more teams adopting it. But it's not the same anti-pattern. With rendered manifests, your main branch stays the only place humans write config (using directories, just like above). CI renders the final YAML and pushes it to read-only environment branches like `env/staging` or `env/prod`. ArgoCD points at those output branches instead of the source. The branches are machine-generated artifacts — nobody commits to them directly and they never merge back to main. That's a completely different animal from "let's use the `staging` branch for staging config and cherry-pick to prod." If you want to try it, ArgoCD's built-in [source hydrator](https://argo-cd.readthedocs.io/en/latest/user-guide/source-hydrator/) and [Kargo Render](https://github.com/akuity/kargo-render) can automate the rendering for you.

Here's what a clean directory structure looks like for your source (main branch) in practice:
...
{{% notes type="info" %}}
Pro Tip: Use Kustomize overlays to keep a shared base and per-environment patches. Your base directory holds the common manifests, and each environment directory contains only the differences. This minimizes duplication while keeping environment boundaries obvious. And in practice, it's not "Kustomize or Helm" — most teams use both in tandem. Kustomize patches work great when you know the values ahead of time (replicas, resource limits). Helm parameterization shines when values depend on the destination cluster and aren't known until deploy time, like an Ingress FQDN or a cloud-specific endpoint. Use Kustomize to keep things DRY, Helm to keep things flexible.
{{% /notes %}}
...
If you're using the rendered manifests pattern from the previous section, this still applies to your source branch. The environment branches (`env/dev`, `env/prod`) are CI-generated output, not places where humans do work. Your trunk-based workflow stays on main. The environment branches just happen to exist alongside it, and that's fine because nobody is branching off them or merging them back.

{{% notes type="info" %}}
Pro Tip: If your GitOps repo has branches older than a week that aren't machine-generated environment branches, treat it as a code smell. Set up automated reminders or CI checks that flag stale branches.
{{% /notes %}}
