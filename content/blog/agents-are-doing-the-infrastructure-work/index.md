...
Governance is keeping pace with autonomy, not lagging behind it: **81%** of survey respondents let agents change production infrastructure, but the overwhelming majority of that is gated — 62% require approval, versus 19% that run autonomously. On the guardrail side, 61% still use manual review gates and 54% use policy as code in CI. That's the honest state of the industry: trust in agents (63% trust them with production changes) is outrunning built guardrails, which is precisely the gap Pulumi Cloud, [Pulumi Insights policy packs](/docs/insights/policy/policy-packs/), and [agent accounts](/docs/administration/organizations-teams/agent-accounts/) exist to close.
...
3. **Agent accounts and human-in-the-loop approval.** [Dedicated agent accounts](/docs/administration/organizations-teams/agent-accounts/) give agents scoped, auditable credentials distinct from any human's, so the 62% of teams who require approval before a production change can keep that gate exactly where they want it, whether the proposer is a person or a process.
