...
* [pulumi/eks](https://github.com/pulumi/pulumi-eks)
    ...
    * Support for [additional security groups](/registry/packages/eks/api-docs/) for node groups to attach to user-specified rules
    * Support for [envelope encrypted secrets](/registry/packages/eks/api-docs/) so that Kubernetes Secrets are encrypted in etcd
    * Support for [alternative authentication](/registry/packages/eks/api-docs/) approaches on the cluster and infrastructure created, such as using AWS named profiles
    * Create [kubeconfig files](/registry/packages/eks/api-docs/) that are scoped to IAM users or roles -- this is used to limit access to the cluster.
    * Use the EKS package on clients operating in [HTTP proxy](/registry/packages/eks/api-docs/) environments, such as corporate proxies.
    * [Default to the latest AMIs](/registry/packages/eks/api-docs/) for node groups, using the latest release from the AWS SSM parameter store.
