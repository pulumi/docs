...
A new hire can take a vague convention and work out what it means from the code around it. An agent fills the gap with a guess. Anthropic's [CLAUDE.md docs](https://code.claude.com/docs/en/memory) ask for instructions "concrete enough to verify," like "API handlers live in `src/api/handlers/`" instead of "Keep files organized." One line in my own Go rules says to wrap errors with `fmt.Errorf("...: %w", err)` and never discard one silently. An agent can follow that and a reviewer can check it, which you can't say about "handle errors properly."
