...
social:
    twitter: |
        ...
        The gap isn't generation, it's changing infrastructure afterward:
    linkedin: |
...
- **Tests.** An agent can write and run a unit test against infrastructure code the same way it tests application code, and check its own change before proposing it. `terraform test` exists, but it exercises a plan rather than the code that produced it, so an agent gets a much narrower signal back.
