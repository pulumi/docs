...
social:
    twitter: |
        New research on AI agents managing cloud infra: Terraform ties for best on day-one provisioning, then drops to 33% success on updates. The gap isn't generation, it's changing infrastructure afterward:
    linkedin: |
        ...
        That's not an argument that HCL is bad. It's an argument that Day 1 (write it once, greenfield) and Day 2 (change it, query it, keep it correct over time) are different problems, and most real infrastructure work is Day 2. General-purpose languages give an agent leverage that a declarative config file can't: types, tests, refactoring tools, a compiler, and programmatic access to live state.
    ...
    bluesky: |
        New research: AI agents tie for best on writing Terraform for the first time, then their success rate on changing it later falls to 33%. The gap is Day 2, not Day 1, and that's a language problem:
---

An AI agent can generate a working Terraform file about as reliably as it can generate a working Python script. The gap shows up afterward: when that file needs to change. A recent academic study found agent success rates on infrastructure updates fall to 33% for Terraform, versus roughly 80% for agents working in a general-purpose language. Day 1 is close to solved. Day 2, where nearly all real infrastructure work happens, is where language choice decides everything.

Pulumi ships HCL natively today, and Pulumi Cloud now hosts Terraform and OpenTofu state directly, so this isn't a pitch to abandon either. It's an argument for being precise about what each format is good at, and why the agentic era raises the stakes on that choice.
...
Not because they can't write it. Provisioning shows they can, at 100% in this study. They struggle with HCL specifically on the tasks that require reasoning about state that already exists: reading a live configuration, understanding what changed since it was written, and producing a correct diff. HashiCorp Configuration Language is a declarative, static format. It has no functions to call to introspect the world, no compiler to catch a typo before it reaches an API, and no test suite an agent can run to check its own work before proposing a change. When an agent needs to reason about a plan it didn't write from scratch, and iterate on it, a static config format gives it very little to hold onto, which is consistent with the paper's own diagnosis: hallucinated fields, deprecated method calls, and errors that only a longer context window would paper over. HCL isn't unreliable because agents "can't write configuration files." It's unreliable on this specific job because Day 2 infrastructure work is a change-management and reasoning problem, not a generation problem, and a declarative DSL doesn't give an agent the scaffolding that kind of problem needs.
...
- **Types.** A type error is immediate, mechanical feedback an agent can act on before ever calling a cloud API. A malformed HCL block or a hallucinated field often isn't caught until `apply` fails, or worse, doesn't fail and just does the wrong thing.
- **Tests.** An agent can write and run a unit test against infrastructure code the same way it tests application code, and check its own change before proposing it. There's no equivalent for a stack of HCL resources.
...
- **Refactoring tooling.** Rename a variable, extract a component, or restructure a module, and an IDE or language server can verify the change compiles. As Pulumi co-founder and CEO Joe Duffy put it, describing why this matters for infrastructure specifically: it's rare to want to refactor across every application at once, "but it happens all the time for infrastructure" ([The New Stack](https://thenewstack.io/pulumi-infrastructure-agent-era/)).
