...
<a href="#type_csharp" style="color: inherit; text-decoration: inherit;">Type</a>
    ...
    <dd>{{% md %}}Specifies how you want AWS WAF Regional to respond to requests that match the settings in a rule. Valid values for `action` are `ALLOW`, `BLOCK` or `COUNT`. Valid values for `override_action` are `COUNT` and `NONE`.
{{% /md %}}</dd></dl>
{{% /choosable %}}
