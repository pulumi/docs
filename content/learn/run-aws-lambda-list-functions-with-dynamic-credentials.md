...
type: learn
url: /what-is/run-aws-lambda-list-functions-with-dynamic-credentials/
...
To get the list of Lambda functions, run the command using `pulumi env run` as shown below, making sure to replace `<your-pulumi-org-name>`, `<your-project-name>`, `<your-environment-name>`, and `<aws-region>` with the names of your own Pulumi organization, ESC environment, and AWS Region, respectively.

```bash
$ pulumi env run <your-pulumi-org-name>/<your-project-name>/<your-environment-name> -- aws lambda list-functions --region <aws-region>
```
